...
// ПРОЗА В «…» — ЦЕЛИКОМ ТЕКСТ, ДВОЕТОЧИЕ В НЕЙ ОБЫЧНЫЙ ЗНАК.
//
// Разрез «имя: текст» шёл по сырой строке, не глядя на кавычки, и «Вывеска
// гласила: закрыто.» становилась репликой говорящего «Вывеска гласила» — тихо,
// без ошибки. Тот же путь советует сообщение компилятора про неизвестную
// команду («write it as dialogue with «…» quoting if this is prose»), то есть
// автор шёл по совету и получал не то.
func TestQuotedProseKeepsItsColon(t *testing.T) {
	doc, err := Convert(`scene t
...
`)
	...
	got := doc.Script
	if len(got) != 4 {
		t.Fatalf("ожидалось 4 команды, вышло %d: %+v", len(got), got)
	}

	// 1. Проза в кавычках — весь текст, говорящего нет, кавычки сняты.
	if got[0]["who"] != nil || got[0]["text"] != "Вывеска гласила: закрыто." {
		t.Fatalf("проза в «…» разобрана как реплика: %+v", got[0])
	}
	// 2. Без кавычек двоеточие по-прежнему делит имя и текст — это язык.
	if got[1]["who"] != "Вывеска гласила" || got[1]["text"] != "закрыто." {
		t.Fatalf("обычная строка должна делиться по двоеточию: %+v", got[1])
	}
	// 3. Обычная реплика не задета.
	if got[2]["who"] != "Anna" || got[2]["text"] != "привет" {
		t.Fatalf("реплика сломана: %+v", got[2])
	}
	// 4. Кавычки ПОСЛЕ имени — по-прежнему реплика этого имени, и двоеточие
	//    внутри неё остаётся текстом.
	if got[3]["who"] != "Анна" || got[3]["text"] != "Пауза: тишина" {
