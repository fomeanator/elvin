...
// проектах (в двух из пяти замеренных) и каждый ТИХО терял сюжет: реплики оставались в
...
func TestJumpFollowsTargetPin(t *testing.T) {
	d := partition(
		...
		node(cidDialogFrag, 20, 10), pinObj(200, 20), pinObj(201, 20),
		frag(20, "g-20", "Ты проиграл. Попробуй ещё раз."),
...
// экспорта целиком (в одном проекте так пропали 7 реплик сцены). Ordinal контейнера
