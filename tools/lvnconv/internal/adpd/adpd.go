...
// swallow the real one behind it. Поймано на живом партнёрского проекта: у 11 связей
...
// одной главе партнёрского проекта (Эпизод 11) из 739 реплик оставалось достижимо 14. Отсюда
...
// проектах (пять частных, в этом репозитории их нет): длина «пробега» resync'а до
...
func entries(d []byte, a, b int) []entry {
	...
	for o < b && o+5 <= b {
		...
		case tag == 0x0a && pid == pConnHash && seq < 0x600 && v+4 <= b:
			...
			// и оказалось хуже: на партнёрского проекта оно съедало pSelf одного контейнера, и из
...
// одном из партнёрских проектов: ровно ОДИН объект из 129813 (Dialog со сценой)
...
//
// ВАЖНО: ordinal 0 — законный (в партнёрского проекта это первая глава), поэтому «известность»
