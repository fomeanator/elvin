...
// ПРИЗНАК УСТУПКИ СНИМАЮТ ОДИН РАЗ — ПРИ РОЖДЕНИИ МЕСТА.
//
// Источник отмены отпускают, когда место возвращают; признак читают и ПОСЛЕ —
// заход, услышавший просьбу, ещё разбирается, уступка это или отмена. Спроси
// он признак у отпущенного источника — получит исключение вместо честного
// «да, просили», и уступка обернётся сбоем.
//
// Поймано прогоном 01.09: два теста уступки легли с ObjectDisposedException
// ровно после того, как просьбу стали гасить домом отмены (он гасит И
// отпускает). Дом здесь не подходит по времени: отпускать рано.
func TestYieldFlagIsTakenOnceAtBirth(t *testing.T) {
	root := repoRoot(t)
	body := stripComments(string(mustRead(t, filepath.Join(root,
		"unity/Packages/com.lvn.engine/Runtime/Content/LvnLanes.cs"))))
	if !strings.Contains(body, "seat.Token = seat.Yield.Token;") {
		t.Error("признак уступки больше не снимается один раз при рождении места")
	}
	if strings.Contains(body, "_seat.Yield.Token") {
		t.Error("признак спрашивают у источника — а его отпускают при возврате " +
			"места, и чтение упадёт вместо честного «да, просили»")
	}
	if !strings.Contains(body, "Lvn.LvnCancel.Retire(seat.Yield)") {
		t.Error("источник уступки перестал отпускаться при возврате места — " +
