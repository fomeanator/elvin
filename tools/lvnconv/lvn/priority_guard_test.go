...
func TestLibraryWarmFollowsThePriorityLadder(t *testing.T) {
	...
	// Ступени можно раскладывать двумя домами: ByRung сортирует пачку, а
	// OfChapterPart отвечает про одну часть — лестница 10.09 спрашивает
	// вторым, разбивая главу на «первый кадр» и «остальное». Страж охраняет
	// СМЫСЛ (порядок по ступеням), а не имя вызова.
	if !strings.Contains(body, "LvnPriority.ByRung(") &&
		!strings.Contains(body, "LvnPriority.OfChapterPart(") {
