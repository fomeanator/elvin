...
func TestSkipsDoNotMultiply(t *testing.T) {
	const budget = 24 // 23 динамических + 1 статический (05.09: пять проверок
	// против НАСТОЯЩЕГО сервера — самолечение кэша, доставка событий, защита
	// каталога, смена аккаунта на одном устройстве и удаление аккаунта —
	// пропускаются без собранного бинаря, ровно как смоук рядом с ними.
