...
func TestSkipsDoNotMultiply(t *testing.T) {
	// 19.09: фактически было 36 мест; добавлены два условия для реального
	// Spine: внешний spine-unity отсутствует в TestHost, production-арт задан
	// только через LVN_SPINE_TEST_JSON. Оба теста отдельно прошли в игровом
	// проекте с runtime и серверным winter_garden (TR-127), без пропусков.
	const budget = 38 // 07.09: две проверки
	// размеров панели центра загрузок written против раскладки, которой в этой
	// среде нет вовсе — панель не раскладывается, ширина ноль. Они падали,
	// выдавая этот ноль за «не влезло в окно», то есть врали о вёрстке. Рядом
	// уже три проверки прокрутки, пропускаемые по тому же признаку: считать
	// раскладку batchmode не умеет, и это свойство среды, а не дефект. 06.09: двенадцать
