...
func TestSkipsDoNotMultiply(t *testing.T) {
	const budget = 28 // 27 динамических + 1 статический (05.09: девять проверок
	// против НАСТОЯЩЕГО сервера — самолечение кэша, доставка событий, защита
	// каталога, смена аккаунта, удаление аккаунта и три подмены ответа
	// (чужой JSON, страница входа в сеть, обрыв на половине) — пропускаются
	// без собранного бинаря, ровно как смоук рядом с ними; девятая играет главу
	// эталонным движком и ждёт путь в LVN_PLAYTHROUGH_LVN — контента для неё в
