...
var knownDeadFields = map[string]string{
	// Подпись кнопки «Во весь рост» в гардеробе. Кнопка жива, но с 08.09 она
	// ЗНАЧОК без слова: ряд разделов не помещался в строку, и подпись рядом с
	// ними выталкивала четвёртый раздел на вторую строку. Поле оставлено, а не
	// удалено, потому что оно уже стоит в живых манифестах: убрать его — значит
	// уронить их разбор ради строки, которую никто не видит.
	"peek_text": "кнопка гардероба стала значком без подписи (08.09)",
}
