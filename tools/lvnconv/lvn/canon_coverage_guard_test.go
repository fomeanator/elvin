...
//
// Порог «ДВА читателя» намеренный: дом, к которому ходят из одного места, ещё
// не понятие, а деталь того места; двое — уже общая вещь. Порог начинали с
// трёх, и при затягивании до двух в карту доехали ещё три дома, включая
// единственный переход «манифест → постановка».
// Рядом, но НЕ то же самое: `TestWorldDistrictIsFullyMapped` сверяет РАЙОНЫ
// целиком — там речь про классы-экземпляры с публичными способами (слой
// выборов, эффекты кадра, гашение актёра), которых этот сторож не видит вовсе:
// он смотрит только на статические дома. Третьего сторожа на эту работу
// заводить не надо — расширяется список районов у того.
func TestEveryLivedInHomeIsOnTheMap(t *testing.T) {
	...
	known := map[string]string{
		...
		"LvnAttribution":  "служба: откуда пришёл игрок",
		"LvnLeaderboard":  "служба: таблицы результатов",
		"LvnLogShip":      "служба: отправка логов с устройства",
		"LvnSpineBridge":  "шов расширения: реализацию приносит пакет Spine",
		"Lvn3DDemoSet":    "встроенный набор-образец, а не понятие",
	}
