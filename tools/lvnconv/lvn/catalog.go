...
func ExtractStrings(d *Doc) []string {
	...
	walk = func(c Cmd) {
		...
		case "cutscene":
			// Имя названной катсцены («cutscene start Показ фаворитов|id»)
			// игрок читает в галерее — значит оно переводится тем же
			// каталогом, что и реплики: ключ — авторская строка, перевод —
			// английская. Сам id остаётся техническим и не переводится, иначе
			// переигровка искала бы катсцену по чужому адресу.
