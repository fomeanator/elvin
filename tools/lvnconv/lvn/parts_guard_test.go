...
// под первыми. Семнадцать подписей живого манифеста партнёрской новеллы оживают
// именно ею.
//
...
func TestRadiusComesFromTheThemeOrShrinks(t *testing.T) {
	const budget = 1 // 01.09: все скругления у ступеней темы. Единственный ноль
	//                  живёт ВНУТРИ дома — LvnChrome.Sharp(el), «у картинки своя
	//                  форма». Раньше тот же ноль стоял у потребителей и был
	//                  неотличим от радиуса, подобранного на глаз; теперь у него
	//                  есть имя, и порог закрывает ровно одну строку — саму роль.
	...
	// РОЛИ СЧИТАЮТСЯ НАРАВНЕ С ОБЩИМ СКРУГЛЕНИЕМ. `RoundTop`/`RoundBottom`
	// заведены 01.09, и шаблон `Round\(` их не видел: после «Round» идёт буква,
	// а не скобка. Числа туда пока не попали, но дыра была бы бесшумной — как
	// та, что 315 отступов увела из-под шкалы вместе с переездом на дом
	// воздуха. Новая роль обязана попадать под то же правило, что и старая.
	numeric := regexp.MustCompile(`Round(?:Top|Bottom)?\(\s*[\w.]+\s*,\s*\d`)
