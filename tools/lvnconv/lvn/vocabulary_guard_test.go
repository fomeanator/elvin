...
// закрытые слова манифеста, согласие («да-нет»).
//
...
func TestСловарьСогласияОдинНаДваЯзыка(t *testing.T) {
	root := repoRoot(t)
	cs := stripComments(string(mustRead(t, filepath.Join(root,
		"unity/Packages/com.lvn.engine/Runtime/LvnBool.cs"))))
	go_ := stripComments(string(mustRead(t, filepath.Join(root, "tools/lvnconv/lvn/frame.go"))))
	...
	if m := regexp.MustCompile(`(?s)case "0", (.*?):\n\s*return false`).FindStringSubmatch(go_); m != nil {
		for _, w := range regexp.MustCompile(`"([^"]*)"`).FindAllStringSubmatch(`"0", `+m[1], -1) {
			goNo[w[1]] = true
		}
	}
