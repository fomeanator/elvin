...
// ОГОВОРКА ТОЛЬКО ПО СВОЕЙ СТРОКЕ.
//
// Обычно оговорку ищут в окне ±4 строки: правильный вызов часто стоит
// соседней строкой. Но если оговорка названа ОБОБЩЁННО («using var»), окно
// превращает её в решето — замолкает любое попадание, рядом с которым
// случился чужой `using`. Такие следы названы здесь и смотрят только свою
// строку.
var tightSkips = map[string]bool{
	"загрузчик заведён без конца жизни": true,
}

var bypassProbes = []probe{
	...
	// «Открыта ли глава» — правило Швейцара, а не арифметика по месту.
	// Собранное у себя, оно уже разъезжалось: свой расчёт «номер не больше
	// достигнутого» рисовал замок на ПЕРВОЙ главе непочатой новеллы — рядом
	// с играбельной кнопкой, потому что забывал про «первая открыта всегда».
	{regexp.MustCompile(`\bnumber\s*<=\s*\w*[Rr]eached|\.number\s*<=\s*\w+\s*\)`),
		"открытость главы посчитана на месте",
		"LvnGatekeeper.ChapterOpen(номер, достигнуто, первая) — или LvnChapterMarks.Of, если нужен вид строки",
		regexp.MustCompile(`LvnGatekeeper\.cs|LvnChapterMarks\.cs|НАРОЧНО`)},
	// Ожидание запроса ОПРОСОМ: «пока не готово — уступи кадр» будит
	// продолжение каждый кадр. Тому, кто считает байты (очередь показывает
	// прогресс) и следит за молчанием передачи, это нужно; короткому запросу
	// без прогресса — нет, и там опрос просто жжёт кадры. Дом умеет оба
	// способа; своя копия цикла не умеет выбрать.
	{regexp.MustCompile(`while \(!\w+\.isDone\)`), "запрос ждут опросом по кадрам",
		"LvnNetWait.CompletedAsync (по событию) или AwaitAsync (когда нужен прогресс и срок молчания)",
		regexp.MustCompile(`LvnNetWait\.cs|НАРОЧНО`)},
	// Базовый адрес без хвостовой косой — правило на одну строку, и потому
	// написанное девять раз в четырёх кварталах. Забывший получает
	// host//v1/… : одни серверы отвечают, другие дают 404, и видно это
	// только на чужом хосте.
	{regexp.MustCompile(`TrimEnd\('/'\)`), "базовый адрес правится руками",
		"LvnUrl.Base(адрес) — одно правило на весь движок",
		regexp.MustCompile(`LvnUrl\.cs|НАРОЧНО`)},
	// Обрезка строки по единицам UTF-16 рвёт пару пополам: половина
	// суррогата — это «□» на экране и неразбираемая строка на проводе.
	// Имена пишет игрок, так что эмодзи в начале — не выдуманный случай.
	// Опасен срез по ЧИСЛУ-ПРЕДЕЛУ, а не по найденному разделителю: первый
	// может прийтись на середину пары, второй — никогда. Первая версия следа
	// не различала их и покраснела на двадцати четырёх разборах адресов.
	{regexp.MustCompile(`\.Substring\(0,\s*(\d+|\w*[Mm]ax\w*|\w*[Ll]imit\w*)\s*\)`),
		"строка обрезана по единицам, а не по буквам",
		"LvnClip.Text (подпись) / LvnClip.Head (провод) / LvnClip.FirstLetter (кружок аватара)",
		regexp.MustCompile(`LvnClip\.cs|НАРОЧНО`)},
	// Видимость окна реплики восстанавливали по ОДНОЙ записи из двух:
	// «игрок припаркован на строке» — не считая, что катсцена держит предмет
	// say. Смена шрифта посреди катсцены возвращала пустое окно поверх кадра.
	{regexp.MustCompile(`SetSayVisible\(_sayUp\)`), "видимость реплики восстановлена по одной записи",
		"SayOnScreen — «припаркован И никем не удержан»; удержание катсцены иначе не видно",
		regexp.MustCompile(`VnStage\.Dialogue\.cs|НАРОЧНО`)},
	// «Есть ли из чего выбирать язык» — решение дома, а не проверка
	// поля. Стояло двумя написаниями, и различала их не мысль, а
	// внимательность: одно место проверяло null, другое нет.
	// Ловим РЕШЕНИЕ по полю, а не присваивание: хост обязан список
	// ставить, и первая версия следа краснела именно на нём.
	{regexp.MustCompile(`AvailableLocales\s*(\.Count|[!=]=\s*null)`), "выбор языка решён по полю",
		"LvnLocale.Offered — «у новеллы есть варианты»; список нужен только тому, кто их перебирает",
		// Без «foreach» в оговорках: слово слишком частое, и в окне
		// контекста оно приходит от СОСЕДНЕЙ строки — мутация показала,
		// что след молчит там, где рядом просто есть цикл. Перебирающему
		// список поле и не нужно: он берёт его в переменную.
		regexp.MustCompile(`LvnLocale\.cs|LvnPrefs\.cs|НАРОЧНО`)},
