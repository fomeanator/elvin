...
func TestSpaceScaleDoesNotSpreadFurther(t *testing.T) {
	const budget = 0 // 01.09: 51 → 0. Сорок два были не отступами, а волосяными
	//                   зазорами (1–5) — им завели две ступени в теме (Hair 2,
	//                   Tight 4): пять чисел на соседних рядах и есть тот
	//                   разнобой, ради которого шкала заведена, только ниже её
	//                   нижней ступени. Шесть — поле СТРАНИЦЫ (30/34), у него
	//                   теперь имя в кромочнике (PageSide). Два не были
	//                   размерами вовсе: 52 повторял LvnEdges.HomeTopMin, а 124
	//                   был чужой высотой — высотой нижнего меню, записанной
	//                   рукой во втором файле (см. LvnEdges.Under).
	//
	//                   ПОРОГ БЫЛ 51 ПРИ СЧЁТЕ 51 — и всё же был разрешением:
	//                   он говорил «столько можно», а список за ним ни разу не
	//                   раскрывали. Раскрытый поимённо, он оказался не долгом,
	//                   а тремя разными вопросами, из которых ни один не был
	//                   про шкалу.
