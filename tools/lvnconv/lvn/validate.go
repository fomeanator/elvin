...
func ValidateExt(d *Doc, ext *ExtGrammar) []Issue {
	...
	if len(defined) == 0 && len(d.Script) >= 40 {
		// Две беды на одну причину, обе замерены 06.09. Вошедший из
		// сохранения вернётся в начало главы (якорю не за что зацепиться).
		// А тот, кто читает её ПРЯМО СЕЙЧАС, при живой правке перечитает
		// реплику: курсор остаётся номером команды, а команды съехали.
		addWarn(-1, "label", fmt.Sprintf(
			"в главе %d команд и ни одной метки: поправите — вошедшие из сохранения вернутся в начало, "+
				"а читающие её сейчас перечитают реплику; поставьте `label` перед крупными сценами, "+
				"и место игрока переживёт правку",
			len(d.Script)))
	}
