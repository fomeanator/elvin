...
## Веб-плеер: подмножество, а не второй рантайм

**Решено 01.09 (Илья).** Паритет веб-плеера с движком перестал быть целью.
Он остаётся песочницей `/play/` с Share-ссылками и одностраничным экспортом —
это путь «визитка → playground → джем» из стратегии языка, — но **новая
операция НЕ обязана приезжать в браузер**. Обязана лишь не ломать его.

Что это значит на практике:

- строка `js` у случая читается как «плеер не давится», **а не** «плеер это
  рисует». Требовать второго — и есть тот паритет, от которого отказались;
- операции вне подмножества плеер отдаёт рендереру как есть (ветка `default`
  в `Player.step`) и не толкует;
- подмножество ОБЪЯВЛЕНО в `ops-owners.json` → `browser.interprets`, и страж
  `TestBrowserSubsetIsDeclaredAndReal` держит список честным: обещать то, чего
  в плеере нет, нельзя. Обратной стороны нет намеренно — плеер вправе уметь
  больше молча.

Отказ от паритета работает только пока подмножество объявлено. Иначе выходит
худшее из двух: обязательства нет, и знания о том, что умеет плеер, тоже нет.

**Известная слепота, записанная нарочно.** `browser-runner.mjs` сверяет только
остановки, переменные и выражения — блок `expect.scene` он не смотрит вовсе.
Сценический случай с `js` проходит, ничего про сцену не проверив. Достраивать
проверку значило бы вкладываться в замороженный рантайм, поэтому дыра
объявлена, а не закрыта: сценические случаи пишутся с `runtimes: ["csharp"]`
и полем `js_skip`.
