...
// `core.js`, `expr.js`, `color.js` и `place.js`, только со снятыми import/export. Значит
...
const core = read("core.js"), expr = read("expr.js"),
      color = read("color.js"), place = read("place.js");
...
for (const [name, src] of [["core.js", core], ["expr.js", expr], ["color.js", color], ["place.js", place]]) {
...
for (const [name, src] of [["app.js", read("app.js")], ["export.js", read("export.js")]]) {
  // ГЛУБИНА ОТКАТА — тоже правило плеера. Список сторожил два имени, и
  // `pushHistory` проскочил: копии уже разошлись (у одной глубина стояла
  // именованной постоянной, у другой числом) и совпадали по везению. Сторожим
  // САМО ПРАВИЛО — обрезку истории, а не имя обёртки: обёртка законна, она
  // просто зовёт дом.
  if (/history\.length\s*>/.test(src)) {
    problems.push(`${name}: обрезка истории отката своя — глубина должна приходить из core.js (pushBeat)`);
  }
