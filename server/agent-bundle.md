...
Отдельная команда, а не функция — **метка конверсии**:
```
track "первый поцелуй"     // ничего не делает в кадре: её работа — попасть в отчёт
```
Обычные метки (`:имя`) для измерений не годятся: их ставят там, где нужен
ПЕРЕХОД, а не там, где хочется мерить, и в импортированной главе они называются
`n26_000000`. `track` даёт воронку «по важным местам» (`/v1/analytics/marks`) —
сколько ЧЕЛОВЕК дошло до каждого и какая среди них доля платящих.
```
```
...
Группа уезжает в props каждого события, так что в отчётах она доступна разрезом:
`?segment=ab:первая_сцена=b`.

⚠ **Но группа переезжает, если ТЕСТ ПОМЕНЯЛИ НА СЕРВЕРЕ.** Замер: смена долей
(она требует поднять версию) переставляет около половины игроков, а выключение
теста переводит в первый вариант ВСЕХ, кто был во втором. Это осознанная плата
за сравнимость данных — но для истории она значит, что читавший ветку «b» может
вернуться в «a».

Если развилка влияет на сюжет дальше одной сцены — **запомните вариант в
переменной у первой развилки** и дальше сверяйтесь с ней, а не с `abtest`:

```
если not тон                       // ещё не зафиксировали
  set key="тон" value=abtest("тон")
// дальше по главе — только `тон`, и он переживёт правку теста
если тон == "b"
```

Для теста цвета кнопки фиксация не нужна; для развилки сюжета — обязательна.
...
| `cutscene` | **A frame with no interface**: hides the dialogue, choices, HUD labels, quick menu and every `ui` tree at once, optionally pushing the camera in (`zoom`, `dur`). A STATE, not an effect — `cutscene off=1` puts everything back and resets the camera. The same switch the long-press art view uses, now reachable from the script. | `on?`, `off?`, `zoom?`, `dur?` |
| `cutscene_mark` | **Names a stretch of the script** so the player can watch it again from the cutscene gallery: `cutscene start Name` … `cutscene end` (an explicit address may follow the name, separated by a vertical bar). Stores an ADDRESS, never a recording — replay re-plays the same script, so nothing new lands on the server. Hides nothing: that is `cutscene`'s job. | `id`, `name`, `end`, `dur`, `poster` |
| `ui` | A whole **interface tree** as real UI Toolkit elements — the flexbox layout is computed at RUNTIME, so wrapped text, list length and screen notch all measure themselves. Nodes: `panel`/`row`/`column`, `text`, `bar`, `icon`, `image`, `button`, `space`, `scroll`. Any field may hold a `{expression}`, re-evaluated every **60 ms**, and only changed values reach the style. Re-declaring the same `id` DIFFS the tree instead of rebuilding it (a rebuild would drop the tap under the finger, the scroll position and any running animation). `layer=hud` (default) sits **under** the dialogue box AND is pushed up by its height, so `at=bottom` means "above the dialogue"; `layer=over` sits above everything; `block=1` swallows taps that miss a button, so a menu does not advance the story behind it. `when=` (`idle` / `say` / `choice` / `always`) ties visibility to what is on screen — `idle` hides the tree while a line is being read (measured by waiting-for-tap, not by the box being visible: the box stays up after the story moved on). Sizes and spacing accept theme SCALE STEPS as well as raw numbers: `size=` `xs` / `sm` / `base` / `lg` / `xl` / `display`, and `pad`/`pad_x`/`pad_y`/`gap` as `1`…`6`. Buttons carry a dark bottom edge and sink into it under the finger — UI Toolkit has no shadows. `ui <id> hide` / `show` / `drop` without a body. In `.lvns` it is a block: `ui hud { … }`. | `id`, `action?`(hide/show/drop), `layer?`, `when?`, `block?`, `tree` |
...
| `bg3d` | **A built 3D set instead of painted art** (Unity, Canvas path). `bg3d id=…` resolves the id through `manifest.sets3d`, streams the current platform's AssetBundle through the normal versioned disk cache, and stands its prefab; later `bg3d` calls without an id just move its camera — one room yields as many angles as the script asks for. `off` returns to flat backgrounds. A still set is filmed once and then costs nothing — the engine only films every frame when the set actually moves (animation, particles, scripts), which it detects itself; `live=1`/`live=0` forces that choice when motion hides in a shader. A `fallback_resource` keeps previously downloaded/offline builds playable. See `docs/server-3d-sets.md`. | `id`/`prefab`/`scene`, `x y z`, `pitch yaw`, `fov`, `dur`, `off`, `live` |
...
| `portal` | **A gate as a LAYER of the scene**, not a post-effect: it draws between the background and the actors, so a character stands IN FRONT of it and can be swallowed by it. `open` 0…1 is how wide it has opened (`dur` glides there); `radius` is its size as a fraction of the frame's shorter side, `x`/`y` where it stands (0…1, y down), `color` its glow. A post-effect could not do this — it lives on the camera and works on a frame the character is already painted into, and without a camera it silently does nothing. | `open`,`x`,`y`,`radius`,`color`,`dur` |
| `camera` | `shake`/`zoom`/`pan`/`reset`. | `action`,`amplitude`,`factor`,`x`,`y`,`duration` |
| `particles` | Weather layer. | `type`=rain/snow, `on`(bool) |
| `fx` | Full-frame multi-effect stack (Canvas path). Optics/grading plus animated atmosphere, combat feedback and cinematic styling. `cinematic` applies a clean filmic curve, cool shadows, warm highlights and a restrained optical vignette without animated grain. Drawn looks: `sketch` (pencil — Sobel edges over paper texture, for a memory), `halftone` (comic print dots on a 15°-rotated grid). Air and water: `heat` (shimmer strongest at the ground), `ripple` (rings spreading from `shock_x/y`), `dust` (motes catching the light — an order slower than snow). `ink` floods the frame from the edges with a torn front (`ink_color`), so a scene change can be a blot rather than a fade. `space` is a black-hole lens centered by `space_x/y`; `space_radius` controls its reach. `portal` opens a gateway at `portal_x/y` (author coords, y down) that swallows the frame: the value is how far it is OPEN (0 shut, 1 wide), `portal_radius` how wide it opens, `portal_color` the light in its throat — animate it with `dur` and a scene change becomes a passage rather than a cut. Sticky fields; `fx off` resets. Legacy `grain=` values migrate to `cinematic=` at runtime and produce no noise. | `vignette cinematic chromatic scanlines pixelate glitch bloom rays rays_x rays_y distort space space_x space_y space_radius space_color saturation contrast tint frost blink invert fog fog_color rain snow embers embers_color blood blood_color poison poison_color shockwave shock_x shock_y speedlines dream sepia posterize letterbox sketch halftone heat ripple dust ink ink_color portal portal_x portal_y portal_radius portal_color dur off` |
| `sfx` | Per-actor or per-layer sprite effects (Canvas path): outline/glow/dissolve, status transformations, silhouette motion and manhwa-style body/weapon auras. `aura_style` presets: `basic/guard/fire/frost/storm/shadow/holy/space/distortion/spirit/ascendant`; explicit aura colours override the preset palette. `aura_size`, `aura_speed` and `aura_density` tune the reach, motion and number of animated tongues/ribbons (`1` preserves the preset). `space` pairs with full-frame `fx space`; `distortion` is a slow black-inside/red-outside broken contour; `spirit` is a dense two-colour glow around the combined silhouette; `ascendant` expands the exterior pass into a wide smoky manhwa field with dark living ribbons. `part` addresses a named composite layer such as `body` or `weapon`; `sfx id=… off` resets every layer. | `id part outline outline_color glow glow_color dissolve flash dark tint tint_color ghost ghost_color petrify hologram hologram_color burn burn_color rim rim_color shake aura aura_style aura_color aura_color2 aura_size aura_speed aura_density blade blade_color lightning lightning_color runes runes_color dur off` |
...
With it, opening a chapter puts the heroine beside a gateway on the LIVE scene
— a layer of the scene itself, not a screen over it, so it never covers her and
survives the stage being cleared. Stepping through dissolves her into the
portal, and only then does the chapter begin. The gateway carries no captions:
it is a place in the world, and the words around it belong to the menu. She keeps
