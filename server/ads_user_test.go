...
// по-разному: запись обязана отказаться при сбое упаковки (иначе пустой файл
// затрёт счёт), а чтение — вернуть карты, в которые можно писать. Запись в
// nil-карту в Go — это паника, то есть отказ всей ручки, а не одного игрока.
func TestAdsUserRoundTrip(t *testing.T) {
	s := &AdsService{dir: t.TempDir()}
