...
#
# ЧТО ИМЕННО ПРИБИВАЕТ СИД. LVN_SOAK_SEED_BASE управляет и ВЫБОРАМИ бота
# (SoakBotTests.SoakOne: `new System.Random(seed)` на выбор опции), и бросками
# САМОГО КОНТЕНТА — rand()/chance() в выражениях идут из потока
# Lvn.LvnExpression.Random, и соук с 01.09 сеет его тем же числом.
#
# До этого сеялись только выборы бота, и один сид давал РАЗНЫЕ прогоны:
# упавший тест переигрывался «примерно похожим» проходом, а таблица ниже не
# отличала «иногда падает» от «контент выбросил другое число». Соук, который
# находит баг и теряет путь к нему, стоит меньше, чем кажется.
#
# Отсюда --seed-base: упавший прогон переигрывается ЕГО ЖЕ сидом, и теперь это
# честно воспроизводит и выборы, и броски.
set -u -o pipefail
