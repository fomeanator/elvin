...
# ШОВ ЛИНКЕРА. Необязательная часть движка цепляется сама, из
# [RuntimeInitializeOnLoadMethod], и ссылок на себя не имеет ни одной — ровно
# поэтому UnityLinker выбрасывает её из сборки. В редакторе вырезания нет, так
# что отказ виден ТОЛЬКО на устройстве и выглядит как «эта штука не работает».
# Замер 07.09: в APK был мост спайна и ни одного типа его реализации.
if [ -x "$REPO_ROOT/qa/linker-seam-check.sh" ]; then
  gout_seam="$OUT/linker-seam.log"
  if "$REPO_ROOT/qa/linker-seam-check.sh" >"$gout_seam" 2>&1; then
    log "шов линкера: $(tail -1 "$gout_seam")"
  else
    log "FAIL: шов линкера — подробности: $gout_seam"
    fail=1
  fi
fi
...
# ЧЕСТНЫ ЛИ САМИ МЕРКИ. Каждый стенд умеет доказать свою зоркость по флагу
# -bite; спрашиваем у всех сразу.
#
# СПРАШИВАЕМ У ВСЕХ, А НЕ У СЕМИ. Здесь стоял быстрый режим с оговоркой
# «полный слишком долог для каждого прогона — гоняйте руками». Оговорка была
# написана без числа, а руками не случилось ни разу: сколько мерок в ней
# участвует, узнали только 07.09, когда отчёт научился это называть — СЕМЬ из
# сорока шести. Тридцать девять зелёных цветов держались на честном слове в
# каждом прогоне.
#
# Число теперь измерено: полный 226 с против быстрого 51 с, то есть три
# минуты на четырнадцатиминутном прогоне. Полный прогон 07.09 дал 46 из 46
# честных, слепых и сломанных ноль, — но завтра это неправда по умолчанию, и
# заметить обязан судья, а не человек, вспомнивший про ручной прогон.
if [ -x "$REPO_ROOT/qa/bites-honest-check.sh" ]; then
  gout_bh="$OUT/bites-honest.log"
  if "$REPO_ROOT/qa/bites-honest-check.sh" >"$gout_bh" 2>&1; then
    log "честность мерок: $(tail -1 "$gout_bh")"
  else
    log "FAIL: честность мерок — подробности: $gout_bh"
    fail=1
  fi
fi
...
if [ -x "$REPO_ROOT/qa/quality-rung-cost-check.sh" ]; then
  gout_qr="$OUT/quality-rung-cost.log"
  if "$REPO_ROOT/qa/quality-rung-cost-check.sh" >"$gout_qr" 2>&1; then
    log "цена ступени: $(tail -1 "$gout_qr")"
  else
    log "FAIL: цена ступени — подробности: $gout_qr"
    fail=1
  fi
fi
...
# ЗАПИСЬ СТАЛА НЕВОЗМОЖНА (диск кончился, том только для чтения, права уехали) —
# раздача обязана продолжаться, а автор обязан узнать об отказе. Стенд отнимает
# право записи у каталога глав и смотрит, что ответит сервер и что станет с
# прежней главой.
if [ -x "$REPO_ROOT/qa/disk-readonly-check.sh" ]; then
  gout_ro="$OUT/disk-readonly.log"
  if "$REPO_ROOT/qa/disk-readonly-check.sh" >"$gout_ro" 2>&1; then
    log "запись невозможна: $(tail -1 "$gout_ro")"
  else
    log "FAIL: запись невозможна — подробности: $gout_ro"
    fail=1
  fi
fi

# ПУБЛИКАЦИЯ ИДЁТ ПО ЖИВОМУ: автор жмёт «сохранить», пока игроки качают эту же
# главу. Увиденная частично запись — оборванный JSON, который ляжет в кэш и
# будет встречать игрока при каждом заходе. Стенд сталкивает записи и чтения.
if [ -x "$REPO_ROOT/qa/atomic-publish-check.sh" ]; then
  gout_ap="$OUT/atomic-publish.log"
  if "$REPO_ROOT/qa/atomic-publish-check.sh" >"$gout_ap" 2>&1; then
    log "запись против чтения: $(tail -1 "$gout_ap")"
  else
    log "FAIL: запись против чтения — подробности: $gout_ap"
    fail=1
  fi
fi
...
# 19.09: прежние 2593 включали 98 приватных server/content/scripts/*.lvn,
# которых нет в Git. Сверка XML: обязательные тесты не исчезли, 2495 те же.
# Внешний soak помечен LvnExternalContent и не может маскировать потерю
# обязательных фикстур; сами внешние тесты по-прежнему обязаны проходить.
FLOOR_EDITMODE=2495
# 140, а не 141: 09.09 из движка убрали матовое стекло (perf(интерфейс):
# «матовое стекло убрано — под окнами простой тон»), и три его проверки ушли
# ВМЕСТЕ С МЕХАНИКОЙ — это не потеря, а следствие. Пол тогда не поправили, и
# прогон с тех пор кричал «проверки ИСЧЕЗЛИ» на каждом запуске. Сверено с
# коммитом до ночи 11.09: там ровно столько же.
FLOOR_PLAYMODE=151
