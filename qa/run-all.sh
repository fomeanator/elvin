...
# -bite, но проверял это только человек и только вручную. Раз в прогон стоит
# спросить у всех сразу: быстрый режим берёт стенды, не требующие сборки
# сервера (полный слишком долог для каждого прогона — гоняйте руками).
if [ -x "$REPO_ROOT/qa/bites-honest-check.sh" ]; then
  gout_bh="$OUT/bites-honest.log"
  if "$REPO_ROOT/qa/bites-honest-check.sh" --quick >"$gout_bh" 2>&1; then
