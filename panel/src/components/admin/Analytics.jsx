...
const SCREEN_RU = {
  ...
  // Слои Режиссёра экранов (LvnScreenDirector): не экраны оболочки, а то, что стоит поверх.
  alert: "Вопрос (алерт)", "quick-menu": "Быстрое меню сцены", "shell-modal": "Модальный экран оболочки",
};
...
const ELEMENT_RU = [
  ...
  // Имена, которые прислали устройства 11–18.09 и которых словарь не знал.
  [/^mtg:([^/]+)$/, (m) => `пилюля валюты «${m[1]}» (шапка)`],
  [/^emo-(.+)$/, (m) => `фишка эмоции: ${m[1]}`],
  [/^download-capsule$/, () => "кружок загрузок"], [/^download-sheet$/, () => "лист загрузок (пустое место)"],
  [/^shop-price$/, () => "цена пака («купить»)"], [/^avatar-screen$/, () => "экран аватарки (пустое место)"],
  [/^stage-tab-home$/, () => "вкладка «Домой» списка"], [/^stage-tab-(\d+)$/, (m) => `вкладка списка №${m[1]}`],
  [/^stage-detail-sheet$/, () => "лист детали (пустое место)"],
  [/^gacha-screen$/, () => "комната круток (пустое место)"], [/^gacha-pool-grid$/, () => "сетка призов набора"],
  [/^gacha-actions$/, () => "ряд кнопок круток (между кнопками)"], [/^gacha-window$/, () => "окно лент (пустое место)"],
  [/^gacha-window-lane$/, () => "лента круток"], [/^gacha-ceremony$/, () => "церемония приза (пустое место)"],
  [/^gacha-blackout$/, () => "затемнение церемонии"],
