...
namespace Lvn.Tests
{
    ...
    public class ReplayVisualsTests
    {
        ...
        /// <summary>
        /// КАТСЦЕНА ВОЗВРАЩАЕТСЯ ВМЕСТЕ С КАДРОМ.
        ///
        /// <para>`cutscene on` прячет реплику, выборы, метки и меню — это
        /// состояние, а не эффект (её собственное описание: «off=1 возвращает
        /// всё на место»). Игрок, сохранившийся ВНУТРИ неё, обязан вернуться
        /// туда же.</para>
        ///
        /// <para>Замер 06.09 дал ноль: в восстановленном кадре команды не было
        /// вовсе, интерфейс возвращался на месте — хотя автор его убрал.</para>
        /// </summary>
        [Test]
        public void КатсценаВозвращаетсяВместеСКадром()
        {
            var (p, s) = Make(@"{""script"":[
                {""op"":""bg"",""sprite_url"":""bg/зал.jpg""},
                {""op"":""cutscene"",""on"":true},
                {""op"":""say"",""text"":""внутри катсцены""},
                {""op"":""say"",""text"":""всё ещё внутри""}
            ]}");
            p.ReplayVisuals(4);
            int n = s.Applied.Count(c => (string)c["op"] == "cutscene");
            TestContext.WriteLine($"команд cutscene в восстановленном кадре: {n}");
            Assert.AreEqual(1, n,
                "катсцена не восстановилась: игрок сохранился при скрытом интерфейсе, "
              + "а вернулся к показанному");
        }
