...
namespace Lvn.Tests
{
    ...
    public sealed class StageColorTests
    {
        ...
        // СМЫСЛОВОЙ ЦВЕТ ОБЯЗАН ОТЛИЧАТЬСЯ — иначе он ничего не значит.
        //
        // «Получилось», «внимание» и «беда» говорят игроку исход действия, и
        // акцент им не родня: акцент — это тон новеллы. Правило было записано
        // в докблоке темы и там же нарушено значением: у полуночной темы
        // тревога стояла РАВНОЙ акценту, у романтической почти совпадала с
        // бедой. Пилюля «внимание» на кнопке того же цвета не предупреждает
        // ни о чём, и заметит это игрок, а не сборка.
        [Test]
        public void СмысловыеЦветаРазличимыВоВсехТемах()
        {
            foreach (var name in new[] { "midnight", "cyber", "romance" })
            {
                var t = LvnTheme.ByName(name);
                var pairs = new (string, Color, string, Color)[]
                {
                    ("тревога", t.Warn, "акцент", t.Accent),
                    ("беда",    t.Bad,  "акцент", t.Accent),
                    ("удача",   t.Ok,   "акцент", t.Accent),
                    ("тревога", t.Warn, "беда",   t.Bad),
                    ("удача",   t.Ok,   "беда",   t.Bad),
                    ("удача",   t.Ok,   "тревога", t.Warn),
                };
                foreach (var (an, a, bn, b) in pairs)
                {
                    float d = Mathf.Abs(a.r - b.r) + Mathf.Abs(a.g - b.g) + Mathf.Abs(a.b - b.b);
                    Assert.Greater(d, 0.25f,
                        $"тема {name}: «{an}» и «{bn}» неразличимы ({d:F2}). " +
                        "Смысловой цвет, совпавший с соседом или с акцентом, " +
                        "не сообщает исхода — экран теряет способность кричать.");
                }
            }
        }
