...
namespace Lvn.Tests
{
    ...
    public class LongChapterCostTests
    {
        ...
        private static long ОдинЗаход(int реплик, int метокЧерез)
        {
        ...
        private static long ПрогонМс(int реплик, int метокЧерез)
        {
            ...
            for (int попытка = 0; попытка < 3; попытка++)
                best = Math.Min(best, ОдинЗаход(реплик, метокЧерез));
            return best;
        }

        /// <summary>
        /// ОТНОШЕНИЕ МЕРЯЕТСЯ ПАРАМИ, А НЕ ДВУМЯ ОТДЕЛЬНЫМИ ЗАМЕРАМИ.
        ///
        /// <para>Прежде брались три захода малой главы, потом три большой, и
        /// делились лучшие. Если нагрузка машины между этими группами
        /// изменилась — а под полным прогоном она меняется всё время, — в
        /// числителе и знаменателе оказывались РАЗНЫЕ условия, и отношение
        /// врало. Замер 06.09: тест дал 9,06 при пороге 8,0 под полным
        /// прогоном и 5 из 5 зелёных на свободной машине через три минуты.
        /// Красный цвет без единого дефекта — худшее, что бывает с меркой:
        /// его перестают читать.</para>
        ///
        /// <para>Пара замеров идёт подряд, отношение считается внутри пары, и
        /// берётся ЛУЧШЕЕ из отношений: медленный отрезок портит обе половины
        /// пары сразу и на их частное почти не влияет.</para>
        /// </summary>
        private static double ОтношениеЦены(int малых, int больших, int метокЧерез)
        {
            double best = double.MaxValue;
            for (int попытка = 0; попытка < 3; попытка++)
            {
                long малая = Math.Max(1, ОдинЗаход(малых, метокЧерез));
                long большая = ОдинЗаход(больших, метокЧерез);
                best = Math.Min(best, (double)большая / малая);
            }
            return best;
        }
        ...
        {
            // Длины разведены вчетверо и взяты крупными НАМЕРЕННО. При 2000 и
            // 6000 прогон после починки укладывается в единицы миллисекунд, а
            // на таких числах гранулярность таймера и шум машины дают разброс
            // больше, чем сам эффект: прогон 05.09 показал 8 против 52 мс и
            // отношение 6,5 при пороге 6 — красный цвет без единого дефекта.
            // Вчетверо длиннее глава: линейно это ×4, квадратично ×16.
            double отношение = ОтношениеЦены(4000, 16000, 0);

            TestContext.WriteLine($"без меток: отношение 16000/4000 = {отношение:F1} "
                                + "(линейно ≈4, квадратично ≈16), лучшее из трёх ПАР");

            Assert.Less(отношение, 8.0,
                "вчетверо более длинная глава стала дороже больше чем вчетверо — цена тапа растёт по ходу чтения");
        }
