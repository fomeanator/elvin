...
namespace Lvn.Tests
{
    ...
    public sealed class ProgressMarkerTests
    {
        ...
        [Test]
        public void ПереимпортНеОтбираетТочкуВНулевойГлаве()
        {
            // Вводная новелла: единственная глава под номером 0. Игрок стоит в
            // ней, переимпорт переименовал главу — id в метке мёртв.
            var было = Title(("pilot", 0));
            LvnProgress.StartChapter(было, было.ChaptersOf()[0]);

            var стало = Title(("pilot_v2", 0));

            // Раньше ноль читался как «номера не записали», выручалочка не
            // срабатывала, и точка терялась. Потолок при этом уцелевал — и
            // карточка честно звала «Продолжить» туда, откуда вход уводил в
            // перезапуск: вся воронка игралась сначала.
            Assert.AreEqual("pilot_v2", LvnProgress.Current(стало)?.id,
                "нулевая глава — такая же глава, и номер выручает её так же");
        }
