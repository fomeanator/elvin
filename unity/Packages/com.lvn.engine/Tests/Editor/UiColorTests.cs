...
namespace Lvn.Tests
{
    ...
    public class UiColorTests
    {
        ...
        public void МусорВПолеКомандыЖалуетсяИНеМеняетЦвет()
        {
            ...
            // «эффект не сработал».
            //
            // Правило НЕ ЗАВИСИТ ОТ ИМЕНИ ПОЛЯ: разбор один на все поля
            // команды. Копия этого теста жила рядом с `burn_color` вместо
            // `color` — и давала ложное впечатление двух независимых проверок,
            // хотя проверяла ту же строку кода. Дублирующий тест хуже
            // отсутствующего: он удваивает цену правки и ничего не добавляет.
            LogAssert.Expect(LogType.Warning, new Regex("lvn-ui"));
