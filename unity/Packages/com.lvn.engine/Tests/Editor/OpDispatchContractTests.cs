...
namespace Lvn.Tests
{
    ...
    public class OpDispatchContractTests
    {
        ...
        static readonly Dictionary<string, string> Sample = new Dictionary<string, string>
        {
            ...
            // Катсцена: проба с ЗУМОМ, потому что без него оп идёт по короткому
            // пути и не трогает камеру — а сломать можно именно длинный.
            { "cutscene", "{\"op\":\"cutscene\",\"on\":true,\"zoom\":1.1,\"dur\":1}" },
            // Пометка для галереи (TR-70): отдельный оп от самой катсцены —
            // кадр без интерфейса и запись прохождения делаются разными
            // командами, и путь у каждой свой.
            { "cutscene_mark", "{\"op\":\"cutscene_mark\",\"id\":\"meet\",\"name\":\"Знакомство\"}" },
