...
namespace Lvn.Tests
{
    ...
    public class HeroPreloadTests
    {
        ...
        /// <summary>
        /// КАДРЫ АНИМАЦИИ ЕДУТ ВМЕСТЕ С ОБЛИКОМ.
        ///
        /// <para>Трек <c>prop: "frame"</c> подменяет спрайт слоя по значению
        /// оси — так сделаны моргание, липсинк, ходьба, смерть. Значения этих
        /// кадров ЧАСТО не объявлены в <c>axes</c>: замер на живом каталоге —
        /// шесть кадровых треков из семи, включая девятнадцать кадров смерти и
        /// восемь кадров ходьбы. Разворот по осям их не видел.</para>
        ///
        /// <para>Пропасть они не могли — их грузит сам показ, — но грузил он их
        /// В МОМЕНТ выхода персонажа, ожидая сети. Очередь для того и есть,
        /// чтобы этого ожидания не было.</para>
        /// </summary>
        [Test]
        public void КадрыАнимацииЕдутВместеСОбликом()
        {
            var каталог = Каталог();
            var героиня = каталог.sprites["героиня"];
            героиня.anim = new Dictionary<string, LvnAnim>
            {
                ["моргание"] = new LvnAnim
                {
                    tracks = new List<LvnAnimTrack>
                    {
                        new LvnAnimTrack
                        {
                            layer = "лицо", prop = "frame", axis = "emotion",
                            keys = new List<object[]>
                            {
                                new object[] { 0f, "smile" },
                                new object[] { 1f, "сомкнуты" },   // ЗНАЧЕНИЯ НЕТ в axes
                            },
                        },
                    },
                },
            };
            героиня.layers[1].id = "лицо";   // тот слой, что подменяет трек

            var облик = LvnParts.OfHero(каталог).Select(p => p.Url).ToList();
            TestContext.WriteLine("облик: " + string.Join(", ", облик));

            Assert.Contains("sprites/hero_face_сомкнуты.png", облик,
                "кадр анимации, которого нет в объявленных осях, не попал в очередь — "
              + "персонаж будет ждать его по сети в момент выхода");
            Assert.IsFalse(облик.Any(u => u.Contains("{")),
                "в очередь попал шаблон — это гарантированный промах");
        }
