...
{
    /// <summary>
    /// Подложка окна без камеры мира: половина интерфейса живёт там, где её нет —
    /// экраны оболочки, редакторские тесты, режим без канваса. Требование одно:
    /// подложка не должна ронять экран и не должна оставлять окно прозрачной
    /// дырой. Матового стекла (размытой копии мира) здесь больше нет — оно
    /// стоило кадров на слабом телефоне; остался тон, и его плотность
    /// по-прежнему приходит силой.
    /// </summary>
    public class UiGlassTests
    {
        ...
        public void NoCameraStillPaintsTint()
        {
            ...
            var tint = host.Q("lvn-glass");
            Assert.IsNotNull(tint, "тон — вся подложка: размытия здесь больше нет");
        ...
        public void StrengthScalesTintAlpha()
        {
            ...
            float half = host.Q("lvn-glass").style.backgroundColor.value.a;

            UiGlass.Apply(host, 1f, new Color(0f, 0f, 0f, 0.8f));
            float full = host.Q("lvn-glass").style.backgroundColor.value.a;
