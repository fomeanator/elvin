...
namespace Lvn.Tests
{
    ...
    public class StagingHomesTests
    {
        ...
        public void MenuHeroine_InTheStore_StandsLeft_OnACloserPlan()
        {
            ...
            // ВИТРИНА ГЕРОИНЮ НЕ УМЕНЬШАЕТ И НЕ РАСТИТ ПОВЕРХ РОСТА. Место ей
            // освобождает композиция (панели столбиком справа), а не масштаб.
            // За два дня число прошло 0.9 → 1 → 1.15 → 1.115 → 1 («не
            // уменьшать», «приближение 15 процентов», затем «мужчины уходят за
            // экран, убрать увеличение» — Илья 08–09.09): приближение множило
            // долю кадра, и фавориты в 1,90 м при сцене 2,00 не влезали.
            // Страж держит смысл: не меньше единицы и не больше её же.
            float side = LvnMenuStage.CastZoomFor(LvnMenuStage.Room.Side);
            Assert.GreaterOrEqual(home, 1f, "витрина героиню не уменьшает");
            Assert.AreEqual(1f, home, 1e-3f, "на главной — свой рост, без приближения поверх метров");
            Assert.GreaterOrEqual(home, side, "на главной она не дальше, чем в гардеробе");
        }
