...
namespace Lvn.Tests
{
    ...
    public sealed class FrameRefitTests
    {
        ...
        [UnityTest]
        public IEnumerator UpdatingOneActorAfterResizeStillRefitsTheOther()
        {
            var frame = new Vector2(3440f, 114f);
            WorldStage.FrameProbe = () => frame;
            _host = new GameObject("refit-two-actors", typeof(RectTransform));
            var stage = new WorldStage(_host.transform);
            var sprites = new List<Sprite> { NewSprite() };
            var first = stage.ApplyActor("first", sprites, Placement.Standing(0.5f));
            var second = stage.ApplyActor("second", sprites, Placement.Standing(0.5f));
            yield return null;
            Assert.IsFalse(stage.RefitToFrame(), "initial placement must not trigger another placement pass");
            Assert.Greater(second.SlotBase.x, 5000f);

            frame = new Vector2(1170f, 2532f);
            stage.ApplyActor("first", null, Placement.Standing(0.5f));
            Assert.IsTrue(stage.RefitToFrame(), "one actor cannot mark the whole cast as resized");
            Assert.AreEqual(540f, first.SlotBase.x, 1f);
            Assert.AreEqual(540f, second.SlotBase.x, 1f);
            Assert.IsFalse(stage.RefitToFrame());
        }
    }
}
