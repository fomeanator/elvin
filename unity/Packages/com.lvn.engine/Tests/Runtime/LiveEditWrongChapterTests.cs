...
namespace Lvn.Tests.Runtime
{
    ...
    public class LiveEditWrongChapterTests
    {
        ...
        /// <summary>
        /// ПОЗДНЯЯ ПРАВКА НЕ ОТМЕНЯЕТ БОЛЕЕ НОВУЮ.
        ///
        /// <para>Опрос сервера идёт раз в две секунды, а обработчик изменения
        /// контента запускается «выстрелил и забыл» и внутри дважды ходит в
        /// сеть. Автор, поправивший главу дважды подряд, порождает две работы
        /// над ОДНИМ адресом — и на экране осядет та, что вернулась последней,
        /// а не та, что новее. Игрок получает исправленную опечатку обратно.</para>
        ///
        /// <para>Адресом это не ловится: глава своя, отличается только
        /// возраст. Нужен порядковый номер правки.</para>
        /// </summary>
        [UnityTest]
        public IEnumerator ПоздняяПравкаНеОтменяетБолееНовую()
        {
            _stage.SetSaveContext(Новелла, "ch3", АдресТретьей);
            _stage.Play(Глава("первая редакция"));
            yield return null;

            // Вторая правка автора вернулась первой…
            var вторая = _stage.ApplyLiveEdit(АдресТретьей, Глава("вторая редакция"), 2);
            Assert.AreNotEqual(VnStage.LiveEdit.Stale, вторая, "стенд: свежая правка не принята");
            yield return null;

            // …а первая, ушедшая раньше, приехала после неё.
            var первая = _stage.ApplyLiveEdit(АдресТретьей, Глава("первая редакция"), 1);
            yield return null;

            Assert.AreEqual(VnStage.LiveEdit.Stale, первая,
                "сцена приняла ПОЗДНЮЮ правку поверх более новой — автор поправил главу дважды, "
                + "а игрок читает первую редакцию");

            // ВОРОТА НЕ ДОЛЖНЫ ЗАПЕРЕТЬСЯ. Отвергнув старую правку, сцена
            // обязана принять следующую НОВУЮ — иначе одна опоздавшая правка
            // остановила бы живое обновление главы навсегда.
            var третья = _stage.ApplyLiveEdit(АдресТретьей, Глава("третья редакция"), 3);
            Assert.AreNotEqual(VnStage.LiveEdit.Stale, третья,
                "после отказа старой правке ворота заперлись — живое обновление главы встало");
        }
    }
}
