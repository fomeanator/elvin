...
namespace Lvn.Tests
{
    ...
    public sealed class DownloadHudDialTests
    {
        ...
        public IEnumerator MiniRing_SitsOnTheDial_AndTheSheetStillGrowsFromTheBar()
        {
            ...
            // The logo may change size after the first layout. Keep the ring
            // inside the capsule and in panel coordinates, including an inset root.
            dial = new Rect(500f, 225f, 85f, 85f);
            hud.Tick(Work(3));
            for (int i = 0; i < 4; i++) yield return null;
            Assert.AreEqual(dial.center.x, capsule.worldBound.center.x, 1f);
            Assert.AreEqual(dial.center.y, capsule.worldBound.center.y, 1f);
            Assert.GreaterOrEqual(ring.worldBound.width, dial.width);
            Assert.LessOrEqual(ring.worldBound.width, capsule.worldBound.width);
            hud.style.left = 0f;
            hud.style.right = 0f;
            for (int i = 0; i < 4; i++) yield return null;
