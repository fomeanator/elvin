...
namespace Lvn.Tests.Runtime
{
    public sealed class ProfileLayoutTests
    {
        ...
        [UnityTest]
        public IEnumerator MinimalFitsEveryDeviceAndHidesOnlyStatsProgressAndAchievements()
        {
            // Правило Ильи (TR-25, ac14dc25): в минимальном профиле нет статов,
            // прогресса и достижений; кружок аватара, отношения, пункты и ID
            // на месте. Подвал достижим прокруткой на всей матрице.
            foreach (var size in Matrix)
            foreach (bool stage in new[] { false, true })
            {
                ...
                var body = p.Q<ScrollView>("profile-body");
                Assert.That(p.Q("profile-avatar"), Is.Not.Null, $"{size}: кружок аватара пропал из минимального профиля");
                Assert.That(p.Q("profile-relation"), Is.Not.Null, $"{size}: отношения пропали");
                Assert.That(p.Q("profile-stat"), Is.Null, $"{size}: статы в минимальном");
                Assert.That(p.Q("profile-achievement"), Is.Null, $"{size}: достижения в минимальном");
                foreach (var label in body.Query<Label>().ToList()) AssertTextFits(label, body, "Minimal");
                body.scrollOffset = new Vector2(0, float.MaxValue);
                yield return null;
                Assert.LessOrEqual(p.Q("profile-footer").worldBound.yMax, body.contentViewport.worldBound.yMax + 2,
                    $"{size}: подвал с ID недостижим прокруткой");
