...
namespace Lvn.Tests
{
    ...
    public sealed class HubStageCompositionTests
    {
        ...
        private VisualElement Root(int width, int height)
        {
            ...
            float scale = width > height ? height / (float)LvnPanel.ReferenceHeight
                : width / (float)LvnPanel.ReferenceWidth;
            ...
            document.rootVisualElement.style.width = width / scale;
            document.rootVisualElement.style.height = height / scale;
        ...
        [UnityTest]
        public IEnumerator SpinIsAboveTheHomeMenuAt1080x1920() => CheckMenuClearance(1080, 1920);

        [UnityTest]
        public IEnumerator SpinIsAboveTheHomeMenuOnAShortScreen() => CheckMenuClearance(1080, 1440);

        [UnityTest]
        public IEnumerator SpinIsAboveTheHomeMenuIn1080pLandscape() => CheckMenuClearance(1920, 1080);

        private IEnumerator CheckMenuClearance(int width, int height)
        {
            ...
            AssertMenuClearance(root, hub);

            // The same view must respond to height changes, not only to boot.
            root.style.height = 1440f;
            yield return Layout(8);
            AssertMenuClearance(root, hub);
            root.style.height = 2400f;
            yield return Layout(8);
            AssertMenuClearance(root, hub);
        }

        private static void AssertMenuClearance(VisualElement root, BrowseHub hub)
        {
            var spin = hub.Q("stage-spin");
            var home = hub.Q("stage-tab-home");
            var topPanel = hub.Q("stage-home-panel");
            Assert.Greater(hub.worldBound.height, 0f, "the hub must have a real screen height");
            Assert.Greater(spin.worldBound.height, 0f);
            Assert.LessOrEqual(spin.worldBound.yMax, home.worldBound.yMin - LvnStageKit.D(4f),
                "the entire spin button must clear the raised home button, including its hit area");
            Assert.GreaterOrEqual(topPanel.worldBound.yMin, LvnEdges.Top(hub) + LvnStageKit.D(70f) - 1f,
                "the column must fit below the header when the screen becomes shorter");
            foreach (var point in new[] { spin.worldBound.center,
                new Vector2(spin.worldBound.center.x, spin.worldBound.yMax - 2f) })
            {
                var picked = root.panel.Pick(point);
                Assert.IsTrue(picked == spin || spin.Contains(picked),
                    $"the spin button must receive the tap at {point}; picked {picked?.name}");
            }
        }
