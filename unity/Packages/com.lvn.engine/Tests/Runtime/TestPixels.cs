...
namespace Lvn.Tests
{
    ...
    public static class TestPixels
    {
        ...
        /// проверка падает и говорит, как его записать; есть — считается
        ...
        public static void AssertGolden(Texture2D shot, string name, float tolerance = 0.01f)
        {
            ...
            if (System.Environment.GetEnvironmentVariable("LVN_GOLDEN_WRITE") == "1")
            {
        ...
        public static void RequireLayout(UnityEngine.UIElements.VisualElement el, string what)
        {
            ...
            {
                string reason = $"панель UITK в этой среде не считает раскладку ({what}) — размеры проверить нечем";
                if (System.Environment.GetEnvironmentVariable("LVN_PREBUILD") == "1") Assert.Fail(reason);
