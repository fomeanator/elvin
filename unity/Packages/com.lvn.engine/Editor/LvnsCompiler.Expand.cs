...
namespace Lvn.Editor
{
    ...
    public static partial class LvnsCompiler
    {
        ...
        /// <summary>
        /// Сколько «…» осталось незакрытыми после этой строки.
        /// Многострочная проза — не код: её `{` и `}` это подстановка или
        /// просто знаки, и ни развороты, ни сбор функций их не считают.
        ///
        /// <para>ЁЛОЧКА ВНУТРИ "…" — ДАННЫЕ, А НЕ СИНТАКСИС. Считать «…» умели
        /// двое — этот счётчик и разбор строк выше, — и знал об этом только
        /// второй. Расхождение давало тихий мискомпил: строка
        /// <c>say text="«Не уходи"</c> проходила разбор как обычная команда, а
        /// развороты блоков считали себя внутри многострочной прозы — и
        /// <c>if …{</c> с <c>}</c> становились РЕПЛИКАМИ, а тело блока играло
        /// безусловно. Ни компилятор, ни импорт при этом не жаловались.</para>
        ///
        /// <para>Клампинг посимвольный (лишняя » в начале строки не уводит
        /// глубину в минус) — так же, как в Go-транскодере.</para>
        /// </summary>
        static int ChevRun(int depth, string s) => ChevScan(depth, s, true);

        /// <summary>Тот же счёт, но БЕЗ клампинга: разбор строк отличает
        /// «строка открыла прозу» от «строка её закрыла», и для этого ему
        /// нужен знак. Зеркалит Go chevronDelta.</summary>
        static int ChevronDelta(string s) => ChevScan(0, s, false);

        /// <summary>
        /// ЕДИНСТВЕННЫЙ СЧЁТЧИК «…» на весь компилятор — зеркало Go chevScan.
        /// Старшинство то же, что у резчика комментариев (StripLineComment), и
        /// это не совпадение: вопрос один — «что здесь код, а что текст».
        ///
        /// <para>1. ВНУТРИ «…» НЕ ДЕЙСТВУЕТ НИЧЕГО: кавычка в многострочной
        /// прозе — знак препинания. Пока она считалась разделителем и здесь,
        /// вторая строка реплики вида «Он крикнул: "Стой! / Не уходи!" …»
        /// прятала закрывающую ёлочку, и глава не собиралась вовсе.</para>
        ///
        /// <para>2. ВНЕ «…» кавычка прячет ёлочки: авторская ёлочка в значении
        /// поля (<c>say text="«Не уходи"</c>) — данные. Иначе развороты блоков
        /// считали себя внутри прозы, и <c>if …{</c> с <c>}</c> становились
        /// репликами, а тело блока играло безусловно.</para>
        ///
        /// <para>3. ОДИНАРНАЯ КАВЫЧКА РАВНА ДВОЙНОЙ: язык принимает обе, и
        /// знал об этом весь компилятор, кроме этого счётчика.</para>
        /// </summary>
        static int ChevScan(int depth, string s, bool clamp)
        {
            char inStr = '\0';
            char prev = '\0';
            foreach (char r in s)
            {
                ...
                {
                    if (r == inStr && prev != '\\') inStr = '\0';
                }
                else if (r == '«') depth++;
                else if (r == '»') { if (!clamp || depth > 0) depth--; }
                else if (r == '"' || r == '\'') inStr = r;
                prev = r;
            }
            return depth;
