...
namespace Lvn.UI
{
    ...
    public sealed partial class VnStage
    {
        ...
        ///
        /// <para>СКЕЛЕТ ЗАБЫВАЕТСЯ ЗДЕСЬ ЖЕ, И ЭТО НЕ ДОБАВКА. У спайна свой
        /// статический кэш, и он держит не только разобранный скелет, но и
        /// МАТЕРИАЛЫ, построенные вокруг конкретных текстур. Обновление
        /// контента эти текстуры заменяет — материал остаётся с мёртвой, и
        /// Unity рисует такой меш СВОЕЙ БЕЛОЙ. На экране это белые
        /// прямоугольники по фигуре.</para>
        ///
        /// <para>Правило уже было записано и применено — но на ОДНОМ пути:
        /// снос сцены (<c>OnDestroy</c>) чистит кэш вместе с выгрузкой, и там
        /// прямо сказано, почему. Живое обновление сцену не сносит, и до этой
        /// уборки не доходило. Замер 07.09: две публикации подряд дали в
        /// журнале «контент сменился — правок 2, каталог тоже», и следом на
        /// устройстве — белые прямоугольники на скелете, БЕЗ единой ошибки
        /// загрузки. Ошибок и не могло быть: файлы приехали, умер материал.</para>
        /// </summary>
