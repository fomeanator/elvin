...
namespace Lvn.UI
{
    ...
    public static class LvnPicture
    {
        ...
        public static void Pin(VisualElement el, Sprite sprite, ILvnAssets assets)
        {
            ...
            // И ВЕРНУЛСЯ — ЗНАЧИТ СНОВА ДЕРЖИМ. Отпускание висело на уходе, а
            // взятие жило в этом способе — то есть случалось, когда картинку
            // СТАВЯТ. Элемент, ушедший с панели и вернувшийся, оставался на
            // экране без пина: спрайт виден, а вытеснитель вправе его выгрузить.
            // Ровно этот путь однажды и дал «белый прямоугольник».
            //
            // Держим то, что показано СЕЙЧАС, а не то, что было при подписке:
            // между уходом и возвратом картинку могли сменить.
            el.RegisterCallback<AttachToPanelEvent>(_ =>
            {
                var now = el.resolvedStyle.backgroundImage.sprite;
                if (now != null) _pins.Hold(el, loader, new[] { now });
            });
            // ПОДПИСКА ОДНА. 02.09 этот блок стоял здесь ДВАЖДЫ — правку внесли
            // второй копией рядом с первой. Второй Hold на том же ключе отпускает
            // набор первого и держит заново, а у доски панели задержки нет: на
            // миг счётчик держателя падал в ноль — то самое окно, против
            // которого правка и заведена. Страж: TestCommentParagraphsAreNotDuplicated.
        }
