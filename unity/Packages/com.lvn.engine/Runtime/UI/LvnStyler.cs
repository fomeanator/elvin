...
namespace Lvn.UI
{
    ...
    public static class LvnStyler
    {
        ...
        /// <summary>ЭТО ВЫБРАНО — обводка выбора, одна на все списки.
        ///
        /// <para>Вопрос «выбрано ли» задают из пяти мест: табы оси, ростер,
        /// карточки нарядов, надетое. Отвечали на него пятью разными парами
        /// чисел — три альфы белого (0.12, 0.15, 0.28) и три пары толщин, — и
        /// все мимо темы: белая грань не меняется, когда меняется всё
        /// остальное. В кибер-теме она чужая, в светлой невидима.</para>
        ///
        /// <para>Ответ один: выбранное носит акцент и грань потолще,
        /// невыбранное — тихую грань темы. Акцент приходит доводом, потому что
        /// автор вправе его переназначить (<c>ui.*.accent_color</c>), а вот
        /// «как выглядит НЕ выбранное» авторского вопроса не имеет.</para>
        /// </summary>
        public static T Chosen<T>(T el, bool chosen, Color accent) where T : VisualElement
        {
            if (el == null) return null;
            LvnChrome.Border(el, chosen ? accent : LvnTokens.Border, chosen ? ChosenEdge : QuietEdge);
            return el;
        }

        /// <summary>Толщина грани у выбранного и у остальных.</summary>
        public const float ChosenEdge = 2.5f;
        public const float QuietEdge = 1.5f;
