...
namespace Lvn.UI
{
    ...
    {
        // ОПОРНОЕ РАЗРЕШЕНИЕ — оно же единица измерения всего интерфейса.
        //
        // Было 1080×1920, и это давало ровно ту болезнь, из-за которой экран
        // читался мелким: на телефоне с плотностью 3 холст в 1080 пикселей —
        // это 360 ТОЧЕК, поэтому кегль «19» у подписи вкладки означал шесть
        // точек, а «58» у заголовка — девятнадцать. Ниже одиннадцати точек
        // подпись перестаёт читаться, и никакая типографика этого не спасает.
        //
        // 720×1280 делает единицу в полтора раза крупнее: та же вкладка
        // становится 9,5 точки, заголовок — 29. Числа в коде не тронуты, потому
        // что менять их пришлось бы в трёхстах местах и всё равно разъехаться;
        // масштаб — одно число, и сцена с оболочкой остаются связаны, как и
        // задумано (см. AttachScaler).
        public const int ReferenceWidth = 720;
        public const int ReferenceHeight = 1280;
