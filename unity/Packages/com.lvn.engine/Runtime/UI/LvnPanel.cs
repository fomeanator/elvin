...
namespace Lvn.UI
{
    ...
    public static class LvnPanel
    {
        ...
        /// </summary>
        /// <remarks>ЗВАТЬ ЭТО ИЗ ЭКРАНА НАСТРОЕК НЕ НАДО. Панель подписана на
        /// <see cref="LvnPrefs.Changed"/>, и запись настройки применяет себя
        /// сама — так же, как громкости, шрифты и темп движения. Ручной вызов
        /// стоял в обоих экранах настроек и был безвреден, но учил правилу,
        /// которого нет: следующая настройка, применённая руками, обновит ТОТ
        /// экран, с которого её меняли, и не обновит второй.</remarks>
        ...
        private static void ApplyUiScaleNow()
        {
            ...
            // Настройка уже зажата при записи; повторный зажим здесь страхует
            // старое сохранение, где значение могло лежать вне нынешних ступеней.
            float k = LvnKnobs.ClampScale(LvnPrefs.UiScale);
