...
namespace Lvn.UI
{
    ...
    public sealed partial class VnStage
    {
        ...
        {
            // ОТМЕТКА «УЖЕ ВЕЗЁМ» СНИМАЕТСЯ ВСЕГДА.
            //
            // Тема пересобирается начисто на каждый вход в главу и на каждое
            // обновление манифеста, и у новой темы шрифт пуст — значит его надо
            // ставить заново. Пока отметка висит, `ResolveFont` считает, что
            // шрифт уже везут, и выходит: авторский шрифт молча пропадал со
            // ВТОРОЙ главы, а при живой синхронизации контента — прямо посреди
            // первой. Заодно снятие возвращает право на повтор после неудачи:
            // офлайн-старт иначе оставлял бы движковое начертание до
            // перезапуска приложения.
            //
            // Строка была потеряна 01.09 при сведении двух загрузок шрифта в
            // один дом: у прежнего тела она стояла в `finally`, и вместе с
            // телом ушла. Тихая потеря `finally` — цена любого слияния, и
            // проверять её надо отдельно от «одинаково ли считается».
            try
            {
                await LoadContentFontCoreAsync(url);
            }
            finally { _fontUrlLoading = null; }
        }

        private async Task LoadContentFontCoreAsync(string url)
        {
