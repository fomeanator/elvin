...
namespace Lvn.UI
{
    ...
    internal sealed class ActorAnimator
    {
        ...
        /// <summary>
        /// ОСТАНОВИТЬ ДОРОЖКУ — вместе с тем, что на ней ЖДЁТ.
        ///
        /// <para>«Остановить дорожку» записано в движке трижды: здесь, в
        /// <see cref="StopTarget"/> и у ТРЁХМЕРНОЙ фигуры. Двое чистили очередь
        /// дорожки (<c>mode=queue</c>), а этот — нет, и правило разошлось молча:
        /// снятый шаг вернулся бы сам после следующего запуска, поверх новой
        /// анимации и без всякого повода.</para>
        ///
        /// <para>Сегодня этот метод не зовёт никто — потому расхождение и не
        /// проявилось. Ему и не полагается ждать своего первого звонящего,
        /// чтобы повести себя иначе, чем два его двойника.</para>
        /// </summary>
        public void Stop(string channel)
        {
            _queue.Remove(channel);
