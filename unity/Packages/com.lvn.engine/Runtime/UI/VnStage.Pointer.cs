...
namespace Lvn.UI
{
    ...
    public sealed partial class VnStage
    {
        ...
        private void HandleTap(Vector2 pos)
        {
            ...
            // ИДЁТ ВЫБОР — ТАП ПО СЦЕНЕ НИЧЕГО НЕ ДЕЛАЕТ (TR-98, Илья 14.09).
            // Такт ждёт выбор, не касание, а «самоисцеление» ниже принимало
            // тап по пустому месту за застрявший такт: Advance заново
            // прогонял показ реплики и вариантов — текст и кнопки мелькали.
            // Выбирают только тапом по варианту.
            if (_choicesVisible) { LogSwallow("идёт выбор — тап по сцене не такт"); return; }
            if (_awaitingTap)
            {
