...
namespace Lvn.UI
{
    ...
    public sealed partial class VnStage
    {
        internal static readonly HashSet<string> ReservedActorFields = new HashSet<string>
        {
            ...
            "fill", "fill_from", "fill_time", "depth", "world", "parent", "world_height", "in3d",
        ...
        /// <summary>Снять ВСЕ текстовые лейблы разом.
        ///
        /// <para>Лейбл живёт, пока его не спрячут по имени, — и это правильно
        /// для HUD, который должен пережить и реплику, и смену фона. Но у сцены
        /// есть границы (конец боя, рестарт дня), на которых от неё не должно
        /// остаться ничего; перечислять там два десятка имён руками — способ
        /// однажды забыть одно и получить полосу здоровья поверх меню.</para></summary>
        private void ClearLabels()
        {
            foreach (var kv in _labelEls) kv.Value?.RemoveFromHierarchy();
            _labelEls.Clear();
            _labelTmpl.Clear();
            _labelAnchor.Clear();
            _labelX.Clear();
            ...
            IReadOnlyDictionary<string, JToken> vars = null)
        {
            ...
            if (cmd["parent"] != null) p.Parent = (string)cmd["parent"];
            if (cmd["world_height"] != null) p.WorldHeight = NumOrNull(cmd["world_height"], vars);
            if (cmd["in3d"] != null) p.In3D = BoolOr(cmd["in3d"], false);
            ...
            IReadOnlyDictionary<string, JToken> vars = null)
        {
            var p = new Placement
            {
                ...
                Parent = (string)cmd["parent"],
                WorldHeight = NumOrNull(cmd["world_height"], vars),
                In3D = BoolOr(cmd["in3d"], false),
