...
namespace Lvn.UI.World
{
    ...
    public sealed class LvnFade : MonoBehaviour
    {
        ...
        ///
        /// <para>ПРОЗРАЧНОСТЬ — КРОМКА ДВИЖЕНИЯ. Когда герой едет вбок
        /// (<paramref name="withMotion"/>), он обязан быть плотным почти весь
        /// путь: полупрозрачная фигура, ползущая через сцену, читается как
        /// призрак. Ей отдаётся короткое окно — начало входа и конец ухода.</para>
        ///
        /// <para>КОГДА ДВИЖЕНИЯ НЕТ — ПРОЗРАЧНОСТЬ И ЕСТЬ ПЕРЕХОД. Уход по
        /// умолчанию теперь `fade`: герой никуда не едет, и то же короткое окно
        /// превращало «растворение» в 38 мс мигания после трети секунды
        /// неподвижного стояния — то есть в рывок, от которого и уходили.
        /// Здесь гашение занимает весь ход.</para></summary>
        public static float OpacityProgress(float time01, bool appearing, bool withMotion)
        {
            // Прогресс всегда идёт от `from` к `to`, поэтому без движения обе
            // стороны считаются одинаково: гашение занимает весь ход.
            float local = !withMotion
                ? Mathf.Clamp01(time01)
                : appearing
                    ? Mathf.Clamp01(time01 / FadeWindow)
