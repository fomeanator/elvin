...
namespace Lvn.UI
{
    ...
    public sealed class LvnPrima
    {
                          ...
                          float nudge = 0f, float lift = 0f)
        {
            ...
            // Нулевой подъём возвращает к базе темы: отсутствие y наследовало
            // высоту главной при переходе в магазин или гардероб.
            if (lift <= 0f) pose["y"] = _stage.Theme?.ActorBaselineY ?? 1f;
