...
namespace Lvn.UI
{
    ...
    public static class LvnLocale
    {
        ...
        /// <summary>
        /// ЕСТЬ ЛИ ИЗ ЧЕГО ВЫБИРАТЬ — показывать ли строку языка вообще.
        ///
        /// <para>Решение одно, а стояло двумя написаниями: игровое меню
        /// проверяло <c>AvailableLocales.Count &gt; 0</c>, настройки оболочки —
        /// то же самое, но ещё и с проверкой на <c>null</c>. Разными были не
        /// намерения, а внимательность авторов, и держались эти строки на том,
        /// что список никогда не бывает пустым по-настоящему.</para>
        ///
        /// <para>Спрашивать надо у дома языка: «выбор есть» значит «у новеллы
        /// больше одного варианта», а не «в каком-то поле что-то лежит».</para>
        /// </summary>
