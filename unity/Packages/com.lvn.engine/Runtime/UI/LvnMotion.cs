...
namespace Lvn.UI
{
    ...
    public static class LvnMotion
    {
        ...
        /// <summary>ПОЛЁТ (SmoothStep) — так летит камера: тронулась, набрала
        /// ход, притормозила у цели. Это кривая ПЕРЕЕЗДА между комнатами
        /// витрины, и она нарочно та же, что у сцены для хода актёра между
        /// слотами (WorldActor.StepSlotMove): страницы, полотно и героиня
        /// летят одним движением, а не «интерфейс уже приехал, героиня ещё
        /// в пути».</summary>
        public static float Glide(float p) => Mathf.SmoothStep(0f, 1f, Mathf.Clamp01(p));
