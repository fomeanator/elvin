...
namespace Lvn.UI
{
    ...
    public struct Placement
    {
        ...
        /// <summary>РОДИТЕЛЬ: id объекта, внутри которого живут координаты этого.
        ///
        /// <para>Без него всё размещается в долях ЭКРАНА — и раскладка, собранная
        /// под портрет, разъезжается на альбомной ориентации: плашка ужимается,
        /// а её содержимое разлетается по краям, потому что «10% ширины» стало
        /// другим числом пикселей. С родителем ребёнок живёт в его прямоугольнике:
        /// <c>x=0.5</c> — центр РОДИТЕЛЯ, <c>width=0.4</c> — сорок процентов его
        /// ширины. Экран меняется — вся группа едет и масштабируется целиком.</para>
        ///
        /// <para>Родитель должен быть размещён РАНЬШЕ ребёнка (как в разметке
        /// документа). Неизвестный id — координаты остаются экранными.</para></summary>
        public string Parent;
        /// <summary>РОСТ В МЕТРАХ мира — для спрайта, привязанного через
        /// <see cref="World"/>.
        ///
        /// <para>Без него размер задаётся долей ЭКРАНА, и фигура не живёт в
        /// сцене: рядом с пятнадцатиметровыми соснами скелет «в 62% экрана»
        /// читается великаном, а при отъезде камеры остаётся того же размера.
        /// С ростом в метрах движок проецирует основание и макушку и берёт
        /// экранную высоту из РАЗНИЦЫ — тогда фигура уменьшается с расстоянием
        /// сама и стоит вровень с деревьями.</para></summary>
        public float? WorldHeight;
        /// <summary>Ставить спрайт ОБЪЕКТОМ В СЦЕНУ (биллборд), а не рисовать
        /// на канвасе поверх неё. Работает вместе с <see cref="World"/>:
        /// перспективу, порядок с декорациями и туман считает рендер набора.</summary>
        public bool In3D;
