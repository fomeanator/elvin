...
namespace Lvn.UI
{
    ...
    ///
    /// <para>Здесь у них один дом. Недуг описывается тремя вещами: как понять,
    /// что он есть, как лечить, и сколько терпеть перед вмешательством
    /// (загрузка имеет право доехать сама — лечить живое значит перебивать
    /// его). Лекарь смотрит не чаще, чем просили, лечит не раньше терпения и
    /// ведёт счёт: <see cref="Journal"/> отвечает одной строкой.</para>
    ///
    ...
    public sealed class LvnHealer
    {
        ...
        public void Watch(string name, Check sick, Cure heal,
                          float period = 1f, float patience = 0f)
        {
