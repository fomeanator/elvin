...
namespace Lvn.UI
{
    ...
    public sealed class VnPanelHost : VisualElement
    {
        ...
        public async Task ShowAsync(VisualElement content)
        {
            ...
            // ПОКАЗ ОТМЕНЯЕТ УХОД — И ОБЯЗАН ВЕРНУТЬ РАМУ В ПОКАЗАННЫЙ ВИД.
            // Скрытие могло быть перебито на середине: оно уходит по своей
            // проверке поколения и НЕ доводит состояние — рама остаётся
            // полупрозрачной и сдвинутой вниз, а IsOpen — истиной. Раньше этим
            // не занимался никто, и окно навсегда зависало приоткрытым; а так
            // как InputBlocked выводится из IsOpen, вся история замирала без
            // единого элемента, по которому можно ткнуть. Это и есть софтлок
            // гардероба: быстрый повтор «закрыть → открыть» его воспроизводит.
            style.display = DisplayStyle.Flex;
            _frame.style.opacity = 1f;
            _frame.style.translate = new Translate(0f, 0f, 0f);
        ...
        public async Task HideAsync()
        {
            ...
            // Перебили — состояние доводит ТОТ, КТО ПЕРЕБИЛ (см. ShowAsync).
            // Здесь нельзя ни закрывать (показ уже идёт), ни оставлять раму
