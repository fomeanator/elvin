...
namespace Lvn.UI
{
    ...
    internal sealed class LvnUiLive
    {
        ...
        public void Bind(VisualElement el, string field, JToken raw)
        {
            ...
            // СЫРОЙ ШАБЛОН НЕ СТАВЯТ НИКУДА, КРОМЕ ТЕКСТА.
            //
            // `{gold}` — осмысленная строка только для подписи: игрок увидит её
            // на мгновение, до первого опроса, и это честнее пустоты. Для всего
            // остального шаблон разбирается КАК ЗНАЧЕНИЕ и даёт мусор: длина
            // `{cardW}` читается нулём — узел схлопывается в невидимую точку;
            // скрытие `{gold < 10}` читается согласием (непустая незнакомая
            // строка в разметке значит «да») — узел исчезает. Опрос это чинит
            // через миг, но только если переменные уже есть; если их ещё нет,
            // элемент остаётся мусорным НАВСЕГДА, и автор видит, что написанной
            // им панели просто нет.
            //
            // Умолчание безопаснее любого разбора: пусть до первого опроса
            // действует то, что поставила раскладка.
            if (!live || field == "text") Set(el, field, str);
        }
