...
namespace Lvn.UI
{
    ...
    internal sealed class LvnUiLive
    {
        ...
        public void Refresh(IReadOnlyDictionary<string, JToken> vars, bool force)
        {
            ...
            {
                // ПЕРЕХВАТ НА ВЕСЬ ШАГ, а не на половину. Он стоял только на
                // вычислении, а постановку значения оставлял снаружи: упади
                // разбор цвета или длины — и обход списка прервался бы, а
                // соседние живые значения замерли. Ровно то, от чего защита и
                // ставилась, только с другой стороны.
                try
                {
                    var now = TextInterpolation.Apply(b.Expr, vars);
                    if (!force && now == b.Last) continue;   // не трогаем зря
                    b.Last = now;
                    Set(b.El, b.Field, now);
                }
                catch { /* сломанное выражение не должно гасить соседей */ }
            }
        }

        /// <summary>
        /// Поставить значение элементу — по имени поля.
        ///
        /// <para>НЕПРОЧИТАННОЕ НЕ СТАВЯТ НИКУДА, КРОМЕ ТЕКСТА. Значение,
        /// в котором остались фигурные скобки, — это НЕ значение: переменной
        /// ещё нет, имя написано с опечаткой, ветка ушла в другую сцену.
        /// В подписи такую строку показать честно (автор увидит `{hp}` и
        /// поймёт), а для всего остального она разбирается КАК ЗНАЧЕНИЕ и даёт
        /// мусор: длина читается нулём — узел схлопывается в невидимую точку;
        /// скрытие читается согласием (непустая незнакомая строка в разметке
        /// значит «да») — узел исчезает вовсе.</para>
        ///
        /// <para>И это не только про первый кадр. Опрос идёт всю главу и на
        /// каком-то такте вернёт нечитаемое снова — тогда панель мигнёт
        /// пропажей посреди сцены. А если переменная не появится вовсе, узел
        /// останется мусорным НАВСЕГДА: автор написал панель, а её нет.</para>
        ///
        /// <para>Умолчание безопаснее любого разбора: пусть действует то, что
        /// поставила раскладка.</para>
        /// </summary>
        public static void Set(VisualElement el, string field, string value)
        {
            if (el == null || value == null) return;
            if (field != "text" && value.IndexOf('{') >= 0) return;
            switch (field)
            {
                case "text":
                ...
                case "value":
                    ...
                    // ШКАЛА ЗНАЕТ СЕБЯ САМА: и правило «заливка — первый
                    // ребёнок», и длительность хода живут в доме шкалы.
                    if (fv != null) Lvn.UI.LvnStyler.BarSet(el, fv.Value);
                    break;
            }
        }
