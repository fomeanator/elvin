namespace Lvn
{
    ...
    public static class LvnOpKind
    {
        ...
        /// <summary>ЕСТЬ ЛИ У КОМАНДЫ ЗРИМЫЙ АССЕТ — третий вопрос к тому же
        /// списку, и он НЕ совпадает с двумя первыми.
        ///
        /// <para>Предзагрузчик спрашивает не «про кого команда», а «стоит ли за
        /// ней картинка, которую можно достать заранее». Ответ почти совпадает
        /// с <see cref="LvnOpSubject.Actor"/> плюс <see cref="LvnOpSubject.Background"/>,
        /// но не полностью: <c>sfx</c> относится к актёру и никакого файла за
        /// собой не тянет.</para>
        ///
        /// <para>Раньше список стоял литеральной тройкой в двух местах одного
        /// файла (<c>VnStage.Preload</c>, строки 48 и 178). Заведут новый вид
        /// команды с картинкой — впишут в одно место, и предзагрузка станет
        /// работать через раз: первый проход её увидит, второй нет. Такое не
        /// выглядит ошибкой в новой команде, выглядит «иногда подтормаживает».</para>
        /// </summary>
        public static bool CarriesArt(string op)
        {
            switch (op)
            {
                case "actor":
