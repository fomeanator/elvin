...
Shader "Hidden/LvnSpriteFx"
{
    Properties
    {
        ...
        [HideInInspector] _FadeRamp ("Fade Ramp x0/invW/enabled/dir", Vector) = (0,0,0,0)
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _FadeRamp; // x0, 1/ширина (экранные px), включена, направление
            ...
            // МАСКА ГАШЕНИЯ В ЭКРАННЫХ КООРДИНАТАХ — суть композитного фейда.
            //
            ...
            //
            // Лечение — считать одну почти бинарную экранную маску для всех
            // слоёв: пиксель либо остаётся в полном исходном композите, либо
            // уже снят. Широкая полупрозрачная полоса здесь НЕ годится: даже с
            // одинаковым коэффициентом она снова заставит одежду пропускать
            // тело. Сглаживаем только кромку шириной около двух пикселей.
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                if (_Fade < 0.999)
                {
                    // Без валидной геометрии безопаснее переключить весь композит
                    // целиком, чем сделать его послойно полупрозрачным и показать
                    // нижнее бельё/тело сквозь одежду.
                    float k = step(0.5, saturate(_Fade));
                    // ЧИСТАЯ НАПРАВЛЕННАЯ МАСКА вдоль ширины героя. Front идёт
                    // чуть дальше обоих краёв, поэтому progress=0/1 означает
                    // действительно полностью скрыт/виден — без россыпи точек.
                    if (_FadeRamp.z > 0.001)
                    {
                        float r = saturate((i.pos.x - _FadeRamp.x) * _FadeRamp.y);
                        if (_FadeRamp.w < 0.0) r = 1.0 - r;
                        float front = lerp(-0.10, 1.10, saturate(_Fade));
                        // Не прямая «полоса загрузки»: один широкий матовый
                        // изгиб занимает около 30% ширины силуэта. Это выглядит
                        // мягче, но сама локальная кромка остаётся непрозрачной
                        // для всех слоёв одновременно — одежда не просвечивает.
                        float y01 = saturate(i.pos.y / max(_ScreenParams.y, 1.0));
                        float matte = (sin(y01 * 3.14159265) - 0.5) * 0.30;
                        r += matte;
                        float edge = max(_FadeRamp.y * 1.5, 0.0005); // ≈1.5 screen px
                        k = 1.0 - smoothstep(front - edge, front + edge, r);
                    }
                    if (k <= 0.004) discard;
                    i.color.a *= k;
                }
