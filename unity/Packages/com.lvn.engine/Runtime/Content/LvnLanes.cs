...
namespace Lvn.Content
{
    ...
    public static class LvnLanes
    {
        ...
        /// кадру.</para></summary>
        public static readonly LvnLane Decoder = MakeDecoder();

        private static LvnLane MakeDecoder()
        {
            var lane = DecoderLane(Lvn.LvnDeviceProfile.Cores);
            return new LvnLane("распаковка", lane.Width, lane.KeptForLive);
        }

        /// <summary>
        /// ШИРИНА РАСПАКОВКИ И ЕЁ БРОНЬ — ОДНО РЕШЕНИЕ, А НЕ ДВА.
        ///
        /// <para>Ширина: на слабом устройстве — одна полоса, на обычном —
        /// прежние три. Ядер спросить не удалось (не главный поток) — берём
        /// прежнее число, чтобы не решать наугад.</para>
        ///
        /// <para>Бронь считается ОТ ШИРИНЫ, и это не украшение. Ширина стала
        /// зависеть от устройства, а бронь осталась записанной единицей — и на
        /// двухъядерном телефоне (ширина ровно 1) бронь съедала единственное
        /// место. Конструктор полосы такое запрещает и бросает исключение — из
        /// статического конструктора <see cref="LvnLanes"/>, то есть НАВСЕГДА:
        /// через <see cref="Wire"/> ходит вся сеть, и любое обращение к ней
        /// после этого падает TypeInitializationException. Замер 07.09 на
        /// двухъядерном устройстве: приложение не смогло скачать манифест ни
        /// разу и вечно показывало игроку «нет связи с сервером» — про сеть,
        /// которая была исправна.</para>
        ///
        /// <para>Полоса шириной в одно место брони не имеет: единственное
        /// место делят живое и фоновое, иначе фон стоит навсегда.</para>
        /// </summary>
        internal static (int Width, int KeptForLive) DecoderLane(int cores)
        {
            int width = cores <= 0 ? 3 : Mathf.Clamp(cores - 1, 1, 3);
            return (width, width > 1 ? 1 : 0);
        }
    }
}
