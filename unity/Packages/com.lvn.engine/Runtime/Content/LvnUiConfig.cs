...
namespace Lvn.Content
{
    ...
    public sealed class CanvasOption
    {
        ...
        /// <summary>
        /// ТОЛЬКО ИЗ КРУТОК (TR-47) — как у вещи гардероба. Полотно есть в
        /// витрине, но не продаётся и даром не достаётся.
        ///
        /// <para>Без этого признака приз круток вёл себя ХУЖЕ, чем продаваемый:
        /// цены у него нет, а «бесплатно» для витрины значит «уже твоё» — фон,
        /// ради которого крутят барабан, надевался в два касания.</para>
        /// </summary>
        public bool gacha;
    }
    ...
    public sealed class BrowseConfig
    {
        ...
        /// <summary>Насколько героиня сдвинута ВПРАВО от своего слота на
        /// главной, доля ширины кадра; дефолт 0.12. Слот остаётся словом сцены
        /// («left»), а сдвиг — композиция витрины: столбик панелей справа.</summary>
        public float? doll_nudge;
        /// <summary>Насколько героиня на главной ПОДНЯТА над нижней кромкой
        /// кадра, доля его высоты; дефолт 0 — стоит на кромке. Якорь у фигуры
        /// в ногах, и по макету партнёра она стоит выше: голова под шапкой,
        /// обрез по коленям над меню. Ростом этого не добиться — рост меняет
        /// размер, а не место.</summary>
        public float? doll_lift;
        /// <summary>Где стоит героиня в МАГАЗИНЕ (слот сцены); пусто — там же,
        /// где на главной.</summary>
        public string store_doll_place;
        /// <summary>Насколько крупнее героиня в магазине против главной;
        /// дефолт 1.05 — «ближе на пять процентов».</summary>
        public float? store_zoom;
        /// <summary>Сколько миллисекунд героиня переходит между комнатами
        /// витрины; дефолт 680.</summary>
        public int? menu_travel_ms;
        /// <summary>За сколько миллисекунд ВСПЛЫВАЕТ лист над комнатой
        /// (гардероб, профиль, магазин, галерея). Пусто — доля перелёта:
        /// движение в меню одно, и второе число выводится из первого.</summary>
        public int? menu_rise_ms;
        /// <summary>
        /// КАРТА КОМНАТ ВИТРИНЫ: имя вкладки (<c>home</c>, <c>store</c>,
        /// <c>wardrobe</c>, <c>profile</c>, <c>gallery</c>, <c>titles</c>) →
        /// точка на полотне, <c>[x, y]</c> долями кадра (x 0 — левый край, y 0
        /// — верх).
        ///
        /// <para>Расстановка — решение композиции: полотно нарисовано под неё,
        /// и у другой игры комнаты стоят по-своему. Пусто — движковый ромб:
        /// профиль слева сверху, главная в центре, гардероб и магазин снизу по
        /// краям, галерея и список — справа сверху.</para>
        /// </summary>
        public Dictionary<string, List<float>> rooms;
        /// <summary>
        /// СЦЕНАРИЙ РЕАКЦИЙ ГЕРОИНИ (TR-66): адрес скомпилированного
        /// <c>.lvn</c>, где метками названы события витрины — <c>on_home</c>,
        /// <c>on_store</c>, <c>on_wardrobe</c>, <c>on_profile</c>,
        /// <c>on_idle</c>, <c>on_tap</c>, <c>on_purchase</c>, <c>on_equip</c>,
        /// <c>on_return</c>, <c>on_launch</c>. Пусто — героиня стоит как
        /// стояла: движок эмоций по именам не знает и знать не должен.
        /// </summary>
        public string reactions;
        /// <summary>Через сколько секунд тишины героине становится скучно
        /// (метка <c>on_idle</c>). Пусто — 10; ноль — бездействие выключено.</summary>
        public float? idle_after;
        /// <summary>
        /// АДРЕС ССЫЛКИ НА ПРОХОЖДЕНИЕ (TR-17): начало ссылки, к которому
        /// приписывается код, — например <c>https://ваш.домен/p?s=</c>. Пусто —
        /// игроку показывают один код: домен продукта движку неизвестен, и
        /// выдумывать его значило бы дать ссылку, которая никуда не ведёт.
        /// </summary>
        public string share_link;
        /// <summary>Блуждание полотна витрины — доля картины, на которую кадр
        /// гуляет вокруг своей точки. Дефолт 0 — полотно СТОИТ: даже медленное
        /// покачивание на большом фоне читается как качка («людей тошнить
        /// будет» — Илья 09.09). Назвать число — включить (0.015 = прежняя
        /// «съёмка с рук»).</summary>
        public float? menu_drift;
        /// <summary>За сколько секунд блуждание обходит круг; дефолт 18.
        /// Без <see cref="menu_drift"/> ни на что не влияет.</summary>
        public float? menu_drift_seconds;
