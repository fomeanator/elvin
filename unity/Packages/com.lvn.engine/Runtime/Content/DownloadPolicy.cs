...
namespace Lvn.Content
{
    ...
    public static class DownloadPolicy
    {
        ...
        /// <summary>
        /// КРУПНЫЙ АРТ ИСТОРИИ — четыре папки, и список у них ОДИН.
        ///
        /// <para>Отвечает на вопрос «стоит ли этот файл того, чтобы им
        /// заниматься»: уменьшать до ступени качества, собирать код для
        /// видеокарты, убирать из памяти на выходе из главы. Вопрос один, а
        /// списков было три, и два из них разошлись.</para>
        ///
        /// <para>Уборка главы (<c>NovelApp.Supply</c>) знала три папки из
        /// четырёх: <c>/spine/</c> в её список не попал. Страницы атласа —
        /// самые крупные текстуры движка (до 8K), и они переживали главу
        /// целиком. Ровно этот же дефект там уже ловили однажды — про
        /// <c>/sprites/</c>, с подписью «послойный облик героини (~240 МБ
        /// декода) не матчился и переживал главу целиком», — и починили ОДНУ
        /// строку списка вместо того, чтобы у списка завёлся дом.</para>
        ///
        /// <para>Второе расхождение тише: уборка сверяла папки С УЧЁТОМ
        /// РЕГИСТРА. Урок про «/Art/Hero.PNG» усвоен здесь (см.
        /// <see cref="HasFolder"/>) и до неё не дошёл — контент с заглавной
        /// буквой в папке не убирался вовсе.</para>
        /// </summary>
        public static bool LargeStoryArt(string url)
            => !string.IsNullOrEmpty(url)
               && (HasFolder(url, "/bg/") || HasFolder(url, "/art/")
                   || HasFolder(url, "/sprites/") || HasFolder(url, "/spine/"));
        ...
        public static string DownscaleVariant(string url)
        {
            ...
            if (!LargeStoryArt(url)) return null;
