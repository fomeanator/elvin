...
namespace Lvn.Content
{
    ...
    public sealed class AssetScheduler
    {
        ...
        {
            // СТУПЕНЬ ОБЪЯВЛЯЕТСЯ ОКРУЖЕНИЮ, А НЕ ЭТОЙ ПОЛОСЕ.
            //
            // Полдня здесь стояло `lane.EnterAsync(LvnRung.CurrentChapter, ct)`
            // с комментарием «ступень объявлена вслух» — и это была неправда.
            // У полос расписания брони нет (keptForLive: 0), ступень им не
            // решает ничего; настоящий заход в сеть идёт ГЛУБЖЕ — через Warm →
            // PrefetchSpriteBytes → GetAsync → LvnLanes.Wire, а тот берёт
            // ступень у окружения. Окружение молчало, умолчание — «живое», и
            // сорок шесть картинок главы садились в полосу сети КАК ЖИВЫЕ:
            // занимали бронь, не попадали в список фоновых и не могли уступить.
            //
            // Хуже: честно объявленные фоны (прогрев скелетов первого кадра,
            // арт сцены) в список попадали — и прогрев-самозванец вытеснял
            // именно их. Приоритеты работали наизнанку.
            //
            // Объявление одно, и стоит оно там, где его читают.
            using var rung = LvnRungScope.At(LvnRung.CurrentChapter);
            var lane = isRequired ? SlotFor(item.Tier, item.Size) : _deferredSlots;
            using var pass = await lane.EnterAsync(ct);
            try
            {
                await Warm(item, ct);
            }
