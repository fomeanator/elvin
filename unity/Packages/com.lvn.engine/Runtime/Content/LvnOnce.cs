    ...
    public sealed class LvnOnce<T> where T : class
    {
        ...
        public async Task<T> GetAsync(string key, Func<Task<T>> make)
        {
            ...
            if (_flying.TryGetValue(key, out var pending))
            {
                ...
                // случалось только потому, что дедупа не было вовсе: каждый
                // качал своё (теряя текстуру проигравшего, но получая её).
                //
                // Ждём чужую работу, а если она сорвалась — делаем СВОЮ. Это
                // ровно та цена, которую дедуп и экономил, и платится она
                // только на сорвавшихся.
                try { return await pending; }
                catch (Exception) { /* чужой срыв: делаем свою попытку ниже */ }
                if (Done.TryGetValue(key, out var late)) return late;
                if (_flying.ContainsKey(key)) return null;   // уже начал кто-то третий
            }
