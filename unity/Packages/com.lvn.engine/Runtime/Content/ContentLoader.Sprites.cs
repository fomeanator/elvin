...
namespace Lvn.Content
{
    ...
    public sealed partial class ContentLoader
    {
        ...
        private async Task<(Texture2D tex, long queueMs)> DecodeTextureOffThreadAsync(string url, CancellationToken ct)
        {
            ...
            // КРОШКА НЕ ХОДИТ ЭТОЙ ДОРОГОЙ.
            //
            // Хитрость сама по себе верная: Unity расшифровывает картинку на
            // своём рабочем потоке, и главный поток не платит за декод
            // 1440×1440 сотнями миллисекунд. Но ждём мы события, которое Unity
            // поднимает НА ГЛАВНОМ ПОТОКЕ, в покадровой обработке, — значит
            // между «работа кончилась» и «мы узнали» лежит остаток кадра.
            //
            // Для крупного арта это выгодный обмен. Для крошки-заготовки —
            // разорительный: 200×256 расшифровывается за единицы миллисекунд,
            // а платим мы границей кадра. В живом логе 02.09 пять крошек
            // одного размера в одном такте дали 31, 32, 498, 506 и 1006 мс —
            // разброс в тридцать два раза, и все крупные числа впритык к
            // длине кадра.
            //
            // Хуже всего это било там, где крошка нужнее всего. Её весь смысл —
            // появиться мгновенно, пока едет крупный арт; заложник кадра
            // появляется вместе с ним, то есть не делает ничего. А тяжёлые
            // кадры — это ровно бут, ровно тот момент, ради которого заготовка
            // и заведена.
            if (url != null && url.IndexOf(DownloadPolicy.QMini, StringComparison.Ordinal) >= 0)
                return (null, 0);   // вызывающий расшифрует сам, здесь и сейчас
        ...
        private async Task<Sprite> DecodeSpriteAsync(string url, string key, CancellationToken ct)
        {
            ...
            // ШАБЛОН СЛОЯ — НЕ ХОЛОДНЫЙ КОД. Адрес с неподставленной осью
            // ({hair}) в сеть не идёт (Fetch), но здесь его промах читался
            // как «код ещё кодируется» — и плитка ждала семь секунд, чтобы
            // получить тот же 404 (TR-142). Такому адресу ждать нечего.
            if (Ktx2Only && DownloadPolicy.RasterForbidden(url) && !DownloadPolicy.IsTemplate(url)
