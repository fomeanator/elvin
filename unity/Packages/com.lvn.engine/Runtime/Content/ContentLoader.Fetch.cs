...
namespace Lvn.Content
{
    ...
    public sealed partial class ContentLoader
    {
                    ...
                    catch (LvnFetchException ex) when (ex.Status == 416)
                    {
                        ...
                        if (whole != null && want != null && Sha256Matches(whole, want))
                        {
                            ...
                            lock (_underway)
                            {
                                ...
                                done.Received = whole.Length;
                                done.Expected = whole.Length;
                            }
                            if (File.Exists(path)) File.Delete(path);
                            File.Move(partPath, path);
                            return whole;
        ...
        /// установившееся состояние необязательной пробы (варианты .ktx2,
        ...
        {
            // «НЕТ НИКОГДА» И «ЕЩЁ НЕ СОБРАЛИ» — РАЗНЫЕ БЕДЫ.
            //
            // На запрос кода сервер отвечает 404 И СТАВИТ ЕГО В ОЧЕРЕДЬ:
            // кодирование крупного файла занимает секунды, ждать его в запросе
            // никто не станет. То есть 404 на .ktx2 значит «зайдите позже», а
            // память о пропаже живёт две минуты — и все пять заходов «зайдите
            // позже» (по полторы секунды) били в неё, не доходя до сервера.
            //
            // Итог в живом логе 02.09: семь секунд ожидания проходили ВПУСТУЮ,
            // после чего фон и ядро портала не показывались вовсе — хотя код к
            // третьей секунде уже лежал на диске сервера.
            //
            // У кодов своя память с правильным смыслом (счёт промахов на
            // адрес, чужой успех будит холодных) — эта им только мешает.
            if (url != null && url.EndsWith(".ktx2", StringComparison.OrdinalIgnoreCase)) return;
            bool first;
            lock (_notFound)
            {
