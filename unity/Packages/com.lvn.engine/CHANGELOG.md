...
### Added
- **`fx` — эффекты всего кадра одной командой.** Липкие поля-галочки
  (виньетка, зерно, насыщенность, контраст, тонировка, блюр, хроматика,
  инверсия, иней, моргание век, туман, дождь, снег, угли, кровь, яд,
  ударная волна, спидлайны, дрёма, сепия, постеризация, леттербокс,
  гравитационная линза) с общим `dur` — плавным переходом любого поля;
  `fx off` сбрасывает стек. Реализовано убер-шейдером на камере.
- **`sfx` — эффекты отдельного актёра или слоя.** Обводка, свечение,
  растворение, хит-флеш, затемнение силуэта, перекрас, призрак,
  окаменение, голограмма, горение, rim-свет, тряска, свечение клинка,
  молнии и руны; адресация конкретного слоя через `part=`.
- **Ауры персонажа** — пресеты `aura_style`: `basic/guard/fire/frost/storm/
  shadow/holy/space/distortion/spirit/ascendant`, с настройкой вылета,
  скорости и плотности (`aura_size/aura_speed/aura_density`).
- **Цельная аура составного персонажа.** Части героя пишут объединённый
  силуэт в stencil, отдельный проход рисует ореол только снаружи — швы
  шеи, лица и одежды больше не считаются внешним краем; невидимая
  дилатированная маска закрывает технические щели между деталями.
- **`ui.stage.speaker_focus = "solo"`** — режим новеллы: на сцене остаётся
  только говорящий, остальные уходят фейдом и возвращаются на своей
  реплике. Снимает главную ошибку авторов-ИИ — забытый `actor … hide`.
- **`clear`** — весь каст со сцены одной командой.
- **`weave`** — проза и поток управления прямо в ветке выбора, схождение
  без именованных меток.
- **Экраны с таймером** — `wait` прерывается нажатием на хотспот, так что
  сцена с ограничением времени не «залипает» до конца ожидания.

### Fixed
- **`z` у объекта** — ключ сортировки, а не индекс ребёнка: руки больше не
  оказываются под персонажем при совпадающих слоях.
- **Кириллические имена переменных** принимаются C#-портом компилятора.
- **`line`** — зарезервированный токен HLSL, из-за которого шейдер тихо
  выпадал из билда.
- **Семь расхождений между рантаймами** (C# ↔ JS) и шестое зеркало словаря
  опов; `clear` заявлен в списке движковых сценических опов.

### Server
- **Rev-гейт манифеста** — `manifest.json` несёт `rev`, запись со старой
  копией отклоняется с внятной инструкцией: параллельные авторы (и агенты)
  больше не затирают чужие правки целиком.
- **Валидация `.lvn` при записи** — ошибка в скрипте не доезжает до игрока.
