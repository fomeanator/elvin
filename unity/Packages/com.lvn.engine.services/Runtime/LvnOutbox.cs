...
namespace Lvn.Services
{
    ...
    public sealed class LvnOutbox
    {
        ...
        /// <summary>
        /// ЗАБЫТЬ ОЧЕРЕДЬ — вместе с игроком, который её накопил.
        ///
        /// <para>Долговечная очередь переживала обряд забвения и уходила на
        /// сервер УЖЕ ПОСЛЕ него: игрок просил себя удалить, а его события
        /// продолжали приходить. Обряд при этом честно рапортовал об успехе.</para>
        /// </summary>
        public void Forget()
        {
            lock (_queue) _queue.Clear();
            try { LvnKeep.Drop(_key); } catch { /* нечего забывать — и хорошо */ }
        }
