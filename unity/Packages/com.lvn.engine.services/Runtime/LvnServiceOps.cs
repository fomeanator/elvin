...
namespace Lvn.Services
{
    ...
    public static class LvnServiceOps
    {
        ...
        {
            // ОТВЕТ ВЫБРАСЫВАЕТСЯ НАМЕРЕННО. «false» тут значит четыре разных
            // вещи, и три из них — норма: нет SDK у хоста, игрок закрыл ролик,
            // ролика не дали. Четвёртая — досмотрел, а начисление не прошло —
            // уходит в аналитику из самого дома рекламы (ad_reward_fail).
            //
            // ЧЕГО НЕ ХВАТАЕТ АВТОРУ: узнать исход в скрипте нечем. Ветка «дал
            // награду / не дал» сейчас невыразима, и история идёт одинаково в
            // обоих случаях. Это пробел ЯЗЫКА, а не недосмотр здесь: чтобы его
            // закрыть, опу нужно писать исход в переменную — решение об этом
            // принимается вместе с автором, а не молча в рантайме.
