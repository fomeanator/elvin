...
namespace Lvn.UI.Screens
{
    ...
    public abstract class LvnOverlayScreen : VisualElement, Lvn.UI.ILvnRedress, ILvnHides
    {
        ...
        /// <summary>
        /// ДЛИТЕЛЬНОСТЬ ПРОЯВЛЕНИЯ И УГАСАНИЯ — ВСПЛЫТИЕ ЛИСТА НАД КОМНАТОЙ
        /// (<see cref="Lvn.UI.LvnMenuStage.RiseMs"/>).
        ///
        /// <para>В витрине два движения и только два: комната переезжает, лист
        /// всплывает. Пока у листа было своё число (темп актёров, 0,2 с), а у
        /// переезда своё (0,68 с), игрок видел в одном меню два несвязанных
        /// движения — «всем движение в меню закрепи» (Илья 09.09, TR-65).
        /// Теперь второе выводится из первого, и правка перелёта ведёт лист за
        /// собой.</para></summary>
        protected virtual float FadeSeconds => Lvn.UI.LvnMenuStage.RiseMs / 1000f;

        /// <summary>Окно в <see cref="FadeSeconds"/> для стража движения
        /// (MenuMotionTests): само свойство защищённое, а проверять надо именно
        /// то число, которым живут листы.</summary>
        internal float FadeSecondsForTest => FadeSeconds;
        ...
        /// ПРОЕЗЖАЕТ магазин — экран проносится через кадр без остановки.
        ///
        /// <para>НЕ ПОДКЛЮЧЁН: навигатор (<c>NovelShell.TabGoTo</c>) возит
        /// вкладки напрямую — уезжает текущая, приезжает целевая, — и
        /// промежуточные через кадр не проносятся. Оживить значит позвать это
        /// для каждой вкладки между текущей и целевой.</para></summary>
        ...
        /// <summary>ПЕРЕЕЗД ОКОНЧЕН — экран стоит на своём месте. Пока он ехал,
        /// его раскладка считалась по смещённым координатам; кто вешает на них
        /// своё (лист гардероба — колонки героев и лиц), пересчитывается
        /// здесь. Пустой по умолчанию: большинству экранов это не нужно.</summary>
