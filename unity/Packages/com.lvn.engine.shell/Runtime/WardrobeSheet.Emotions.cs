...
namespace Lvn.UI.Screens
{
    ...
    ///
    /// <para>Правила темы, каждое из живого репорта: лицо НАДЕВАЕТСЯ ТАПОМ
    /// и переживает лист — героиня в меню стоит с выбранным лицом (TR-82:
    /// «выбранная эмоция не переносится в главное меню»), а лицо по умолчанию
    /// снимает выбор и возвращает лицо реакциям витрины; «Выбрать» лица не
    /// касается — фишка бесплатна; обычного скроллбара нет, потому что горизонтальный ряд
    ...
    public sealed partial class WardrobeSheet
    {
        ...
        private void RebuildEmotions()
        {
            ...
            foreach (var v in vals)
            {
                ...
                var chip = new Button(() =>
                {
                    // ЛИЦО НАДЕВАЕТСЯ, А НЕ ПРИМЕРЯЕТСЯ (TR-82, Арам 14.09:
                    // «выбранная эмоция не переносится в главное меню»). Примерка
                    // снималась вместе с листом — героиня в меню возвращалась к
                    // лицу по умолчанию, и выбор читался как невыполненный.
                    // Фишка бесплатна, «Выбрать» ей не нужен: надеваем сразу, и
                    // выбор переживает лист. Лицо по умолчанию — «снять»: тогда
                    // лицом снова владеют сценарий и реакции витрины (LvnFace),
                    // а выбранное лицо сильнее любой реакции.
                    LvnWardrobe.Equip(_entity, _emotionAxis, IsDefaultFace(value) ? null : value);
