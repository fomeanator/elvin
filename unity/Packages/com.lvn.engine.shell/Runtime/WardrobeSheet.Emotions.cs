...
namespace Lvn.UI.Screens
{
    ...
    ///
    /// <para>Правила темы, каждое из живого репорта: в МЕНЮ лицо НАДЕВАЕТСЯ
    /// ТАПОМ и переживает лист — героиня стоит с выбранным лицом (TR-82), а
    /// лицо по умолчанию снимает выбор и возвращает лицо реакциям витрины; в
    /// ГЛАВЕ фишка — примерка поверх сценарной эмоции, пока лист открыт
    /// (TR-92); «Выбрать» лица не касается — фишка бесплатна; обычного скроллбара нет, потому что горизонтальный ряд
    ...
    public sealed partial class WardrobeSheet
    {
        ...
        /// пилюли кошелька слева. Тап надевает лицо на живую куклу сразу и
        /// насовсем: «Выбрать» ему не нужен, и лист его не снимает — выбранное
        /// лицо уезжает с героиней в главное меню (TR-82).</para>
        ///
        ...
        private void BuildEmotionColumn()
        {
            ...
            // тот же приём bottom:100%). Тап надевает эмоцию на живую куклу
            // через Equip оси `emotion` — сразу, без «Выбрать» (см. фишку
            // ниже). Горизонтальный ряд в листе «странно скроллился»
        ...
        private void RebuildEmotions()
        {
            ...
            foreach (var v in vals)
            {
                ...
                var chip = new Button(() =>
                {
                    // В МЕНЮ ЛИЦО НАДЕВАЕТСЯ, В ГЛАВЕ — ПРИМЕРЯЕТСЯ.
                    //
                    // Меню (TR-82, Арам 14.09: «выбранная эмоция не переносится в
                    // главное меню»): примерка снималась вместе с листом, героиня
                    // возвращалась к лицу по умолчанию, и выбор читался как
                    // невыполненный. Фишка бесплатна, «Выбрать» ей не нужен:
                    // надеваем сразу, выбор переживает лист. Лицо по умолчанию —
                    // «снять»: лицом снова владеют сценарий и реакции витрины.
                    //
                    // Глава (TR-92, Илья 14.09: «в гардеробе главы 0 эмоции не
                    // грузятся»): лицом в истории командует сценарий, и надетое
                    // его не перебивает — так и должно быть, иначе героиня играла
                    // бы всю главу одним лицом. Поэтому в сюжетном листе фишка —
                    // примерка: она сильнее сценарной эмоции, пока лист открыт
                    // (LvnCostumer.Look), кукла перестраивается и догружает арт,
                    // а закрытие листа возвращает лицо сценарию.
                    if (TabMode) LvnWardrobe.Equip(_entity, _emotionAxis, IsDefaultFace(value) ? null : value);
                    else LvnWardrobe.Preview(_entity, _emotionAxis, value);
        ...
        /// <summary>Лицо по умолчанию набора: его выбор = снять надетое, чтобы
        /// героиня снова жила реакциями, а не стояла с одним лицом.</summary>
        private bool IsDefaultFace(string value)
            => _def?.defaults != null && _emotionAxis != null
               && _def.defaults.TryGetValue(_emotionAxis, out var d) && d == value;
