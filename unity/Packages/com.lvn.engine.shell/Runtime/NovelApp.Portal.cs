...
namespace Lvn.UI.Screens
{
    ...
    public partial class NovelApp
    {
        ...
        private async Task ArriveInChapterAsync()
        {
            ...
            // ГОТОВЫЙ КАДР НЕ ПЕРЕОДЕВАЮТ.
            //
            // Приезд задуман для НАЧАЛА главы: сцена пуста, героиня выходит из
            // створа, уходит, и история начинается с чистого листа. При
            // возврате с сохранения кадр уже собран реплеем — в нём стоит тот,
            // на ком игрок остановился. Церемония накрывала его собой: спрятать
            // собеседника, показать героиню, убрать героиню, вернуть
            // собеседника — четыре растворения по 0,2 с и почти секунда паузы
            // ради кадра, который и до неё был правильным. Живьём это читалось
            // как «второй запуск дольше первого», хотя запуск был вдвое
            // быстрее: дольше стала возня ПОСЛЕ него.
            //
            // Створ всё равно закрывается: игрок прошёл сквозь него, и оставить
