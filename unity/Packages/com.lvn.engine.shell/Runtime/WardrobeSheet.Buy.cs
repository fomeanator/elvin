...
namespace Lvn.UI.Screens
{
    ...
    public sealed partial class WardrobeSheet
    {
        ...
        private void SetConfirmText(string text, string currency = null)
        {
            ...
            _confirmCoin = LvnPriceTag.Icon(currency, 26f);
        ...
        private async Task BuyCurrentAsync(string axis, LvnWardrobeItem item)
        {
            ...
            {
                _buying = false;
                // БЕЗ ВЪЕЗДА (TR-99): карточки обновляются на месте, а ряд
                // поднастроек (основа, цвет волос) пересобирается — с анимацией
                // он «перерисовывался при покупке». Бейджи меняются тихо.
                RebuildStrip(animate: false); // бейджи «◆» на карточках и свотчах устарели
                RefreshConfirm();
                return;
            }
