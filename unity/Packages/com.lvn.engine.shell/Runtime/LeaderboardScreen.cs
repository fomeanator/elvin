...
namespace Lvn.UI.Screens
{
    ...
    public sealed class LeaderboardScreen : LvnOverlayScreen
    {
        ...
        {
            // ВКЛАДКИ ПЕРЕСМАТРИВАЮТСЯ ЗДЕСЬ, а не только при переключении:
            // пересборку зовёт хост, подставив живую доску, и до неё
            // переключатель ещё имел смысл (демо), а после — уже нет.
            SyncTabs();
            BuildPodium();
            BuildList();
        }
