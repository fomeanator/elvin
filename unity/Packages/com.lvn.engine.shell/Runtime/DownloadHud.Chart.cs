    ...
    internal sealed class TrafficChart : VisualElement
    {
        ...
        public void Add(double now, float downBytes, float upBytes)
        {
            ...
            if (dtAdd > 0.05f)
            {
                ...
                // Без хоста край кривой берётся из самих замеров: величина
                // (кадры, память) — как есть, поток — скоростью, сглаженной.
                if (!_liveExternal)
                {
                    float downNow = Averaging ? Mathf.Max(0f, downBytes) : Mathf.Max(0f, downBytes) / dtAdd;
                    _live = _live <= 0f ? downNow : Mathf.Lerp(_live, downNow, 0.35f);
                }
            }
            else if (!_liveExternal && Averaging) _live = Mathf.Max(0f, downBytes);
        ...
        public void SetLive(float bytesPerSec) { _live = Mathf.Max(0f, bytesPerSec); _liveExternal = true; }
        private bool _liveExternal;   // край ведёт хост; иначе — сами замеры (кадры, память)
        ...
        private void Draw(MeshGenerationContext mgc)
        {
            ...
            float target = Mathf.Max(Floor, PeakDown, PeakUp, _shownDown, _shownUp);
