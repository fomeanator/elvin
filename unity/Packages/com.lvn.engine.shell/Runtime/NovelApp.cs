    ...
    public sealed partial class NovelApp : MonoBehaviour
    {
        ...
        private string OpenContentAndState()
        {
            ...
            if (OfflineBundled)
            {
                ...
                LvnLog.Info($"[lvn-app] offline bundle → {contentBase}");
            }
        ...
        private void PrepareStage(LvnManifest manifest, System.Action<string> mark = null)
        {
            if (Stage == null)
            {
                // Бургер несёт единый навбар оболочки, а не сцена. Флаг
                // ставился при подключении навбара — ПОСЛЕ сборки сцены, и её
                // фаб-меню успевал родиться: в витрине справа висел лишний «≡».
                // Сцена читает флаг в конструкторе, значит ставить его надо здесь.
            ...
            // ВИТРИНУ ГРЕЕМ, ТОЛЬКО ЕСЛИ ЕЁ ПОКАЖУТ.
            //
            // Прогрев стоял безусловно, а ворота вводной спрашивались на
            // четыреста миллисекунд позже — в DriveBootVeilAsync. У НОВОГО
            // игрока витрины не будет вовсе: он уходит в воронку. И всё это
            // время полотно (2000×1500, 3,4 МБ) и четыре слоя куклы занимали
            // места в полосе сети и в полосе распаковки — той самой, где в этот
            // же миг стоят четыре критичных ассета главы 0.
            //
            // Живой трейс первого входа (04.09): полотно 99 мс, кукла 4 слоя,
            // ядро створа 59 мс — и всё выброшено, экран их не увидел.
            //
            // Ответ доступен ЗДЕСЬ: манифест прочитан, а метка вводной лежит в
            // памяти устройства. Спрашивать позже было нечем оправдано.
            bool toFunnel = LvnIntro.Pending(manifest) != null;
            if (!toFunnel) WarmMenuCanvas();   // полотно витрины — к первому же показу меню
            // Лекарь смотрит всегда: он и так молчит, пока мы не в меню, а
            // выключенный сторож не сторож.
            WatchMenuBackdrop();
            ...
            TaskCompletionSource<bool> entryDone)
        {
            ...
            while (Stage != null && !Stage.HasBackdrop && Lvn.LvnClock.Wall() < revealDeadline)
                await Task.Yield();
            LvnLog.Trace($"[lvn-app] entry reveal: backdrop={Stage?.HasBackdrop} " +
        ...
        private void ApplyDeepLink(string url)
        {
            ...
            if (!string.IsNullOrEmpty(chapterId))
            {
                ...
                Debug.LogWarning($"[lvn-app] диплинк на главу ещё не поддержан " +
                                 $"(chapter={chapterId}) — открываю новеллу {titleId} с начала");
            }
            if (!_shell.RequestPlay(titleId))
                Debug.LogWarning($"[lvn-app] диплинк: новеллы {titleId} нет в манифесте");
        }
