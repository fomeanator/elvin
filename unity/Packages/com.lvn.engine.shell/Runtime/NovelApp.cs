        ...
        private void ShowMenuScene()
        {
            ...
            if (!string.IsNullOrEmpty(fav))
                Stage.ApplyStage(new Newtonsoft.Json.Linq.JObject
                {
                    ...
                    ["position"] = "center",
                    // Единый рост — РАЗМЕРЫ МАНЕКЕНА ГАРДЕРОБА (0.92/1.06):
                    // липкая постановка (Вика — мелкая из старых прогонов)
                    // перекрывается явными размерами. Y НЕ задавать: в
                    // Placement Y — якорь НОГ в долях экрана, где 1 = НИЗ;
                    // y=0.02 ставил ноги на 2% от ВЕРХА — актёр целиком
                    // уезжал за кадр («героиня пропала», живой скрин).
                    ["width"] = 0.92, ["height"] = 1.06,
                });
            _menuSceneActor = fav;
        }
