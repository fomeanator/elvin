...
namespace Lvn.UI.Screens
{
    ...
    public sealed partial class NovelShell : MonoBehaviour
    {
        ...
        public void Build(LvnManifest manifest, ILvnAssets assets)
        {
            ...
            // ДОКУМЕНТ — ВО ВЕСЬ ЭКРАН, А НЕ ДОЛЯ КОЛОНКИ. Панель у оболочки и
            // сцены одна, и Unity 6000.4 кладёт корни документов в неё как
            // обычных детей колонки: два растущих корня делили экран пополам —
            // оболочка стояла в нижней половине, сцена в верхней (снимок ui-lab
            // 08.09). Слоение документов — по sortingOrder, а не по потоку.
            LvnChrome.Stretch(_root);
            // …НО НЕ ШИРЕ ТЕЛЕФОНА: на планшете полоса оболочки встаёт по
            // центру, а сцена под ней остаётся во всю ширину.
            ScreenUi.PhoneColumn(_root);
            ...
            // ТЕМА — ДО ШРИФТА. Тему выбирал только хаб, в своём конструкторе,
            // а гарнитура корню ставилась РАНЬШЕ него — и корень получал шрифт
            // прежней темы: у темы со своей гарнитурой («chrono») подписи
            // выходили чужим шрифтом, а карусель без хаба темы не получала
            // вовсе. Витрина по-прежнему вправе перевыбрать тему сама — это
            // тот же выбор, сделанный вовремя.
            LvnTheme.Use(LvnTheme.ByName(ui.browse?.theme));
            ...
            if (assets is CachingAssets ca)
            {
                ...
                // Эта строка терялась при сведении 09.09 — и кружок стоял
                // бабликом поверх слова TIME; связь держит DownloadHudDialTests.
