...
namespace Lvn.UI.Screens
{
    ...
    internal static class ScreenUi
    {
        ...
        /// <summary>
        /// ИНТЕРФЕЙС — ПОЛОСОЙ С ТЕЛЕФОН, ПО ЦЕНТРУ. Оболочка нарисована на
        /// холсте 1080: шапка с логотипом, нижняя лента, столбики панелей,
        /// листы гардероба и магазина. На планшете холст тянется на всю ширину
        /// экрана, и всё это растягивается вбок — надпись логотипа расплывается,
        /// лента становится длинной кишкой, карточки разъезжаются («все
        /// растянуто, надо задать максимальную ширину как на телефоне» — Илья
        /// 09.09).
        ///
        /// <para>Сцена под интерфейсом остаётся во всю ширину: полотно и фигуры
        /// живут своим слоем и вписываются по аспекту сами («фон по идее можно
        /// как есть показывать»). Ограничение — только для полосы оболочки.</para>
        /// </summary>
        ...
        /// <summary>ТИХАЯ ПОДПИСЬ — пояснение под чем-то: приглушённый тон и
        /// ПЕРЕНОС по словам.
        ///
        /// <para>Три строки, но лепили их руками в двух десятках мест, и
        /// главная — перенос. Забудь его, и длинное пояснение уезжает за край
        /// экрана одной строкой: на разработческом широком окне не видно, на
        /// телефоне видно сразу. Это не мелочь вёрстки, а самый частый способ
        /// потерять хвост фразы.</para>
        ///
        /// <para>Кегль приходит доводом, и это не лень: у подписи карточки он
        /// мельче, у подсказки в настройках крупнее, у подзаголовка ленты ещё
        /// крупнее — роль разная. Цвет тоже, потому что новелла вправе
        /// переназначить его своим полем; умолчание — приглушённый тон
        /// темы.</para>
        ///
        /// <para>Отступ НЕ здесь. Подзаголовок жмётся к заголовку, подсказка
        /// под строкой настроек стоит просторнее — это решение места, а не
        /// свойство подписи.</para>
        /// </summary>
        public static T Quiet<T>(T label, float size, Color? ink = null) where T : TextElement
        {
            if (label == null) return null;
            label.style.color = ink ?? LvnTokens.TextDim;
