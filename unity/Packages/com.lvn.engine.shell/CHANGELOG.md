...
### Added
- **Реальные статы игрока** в игровом HUD и панели статов, с подсказками
  о начислениях.
- Гайд для разработчиков оболочки — `docs/ui-development.md` в репозитории
  движка: карта всех экранов, запуск за пять минут, бэклог.

### Fixed
- **Автовозобновление после выбора** — пропуск (skip) продолжается сам,
  как ожидает игрок.
- **Гардероб**: при нехватке валюты открывается попап пополнения вместо
  надписи на кнопке.
