...
voice "/content/voice/a1.ogg"  // озвучка СЛЕДУЮЩЕЙ реплики (авто-стоп на новой)
voice "/content/voice/a1.ogg"  // озвучка СЛЕДУЮЩЕЙ реплики (авто-стоп на новой)
...
choice timeout=10 timeout_goto=late       // ⏱ таймер СЛЕДУЮЩЕГО меню: полоса
- Успеть -> ok                            //   над кнопками, истёк → late
input var=name prompt="Кто ты?" default="Гость" max=24   // ввод текста → {name}
...
- `hint text="…" duration=6` — всплывающее окно сверху по центру; `show=false` убирает, `duration>0` — авто-скрытие. `cost`/`requires_stat` сами ресурсы **не** списывают — списывай явно через `set`/`inc`.
- Нет `ceil` (округляй арифметикой). Таймер и ввод текста ЕСТЬ: `choice timeout=` и `input var=`.
