...
| `bg3d` | **A built 3D set instead of painted art** (Unity, Canvas path). `bg3d id=…` resolves the id through `manifest.sets3d`, streams the current platform's AssetBundle through the normal versioned disk cache, and stands its prefab; later `bg3d` calls without an id just move its camera — one room yields as many angles as the script asks for. `off` returns to flat backgrounds. A still set is filmed once and then costs nothing — the engine only films every frame when the set actually moves (animation, particles, scripts), which it detects itself; `live=1`/`live=0` forces that choice when motion hides in a shader. A `fallback_resource` keeps previously downloaded/offline builds playable. See `docs/server-3d-sets.md`. | `id`/`prefab`/`scene`, `x y z`, `pitch yaw`, `fov`, `dur`, `off`, `live`, `build`, `sway`, `sway_speed`, `walk`, `focus`, `dof`, `dof_range` |
| `o3d` | **A body in the 3D scene, written by the script** (Unity, Canvas path). A primitive (`shape=`), a model from the standing set (`model=`), or a flat billboard (`sprite=`). `bg3d build=1` (or the first `o3d`) opens an empty space, so a place can consist entirely of boxes and planes — no Unity, no artist. One line can sow a whole grove: `count=` copies over `area=` with `gap=`, `scale_var=`, `yaw_var=` and a fixed `seed=` (same grove every run); `kinds=`/`colors=` mix several species and tints so the grove does not read as a pattern; `at="x,z;x,z"` places copies exactly (this is what maps compile to). Copies share one mesh and one material, so a hundred trees cost about one draw call. Motion lives in the same command: `dur=` glides to the new state, `spin=`/`bob=`/`pulse=` run forever, `dissolve=0…1` burns the body away. Look comes from the engine's shader catalog (`shader=` — toon, wind, aura, water, glass, dissolve, road, unlit) — that catalog IS the game's style, and any imported material is converted into it. See `docs/3d-language.md`. | `id`, `shape`/`model`/`sprite`, `pos`, `size`, `height`, `pitch yaw roll`, `color`, `texture`, `alpha`, `glow`, `ground`, `shadow`, `flip`, `shader`, `count`, `area`, `gap`, `seed`, `scale_var`, `yaw_var`, `kinds`, `colors`, `at`, `wind`, `normal`, `bump`, `tiling`, `edge`, `ruts`, `wet`, `dur`, `dissolve`, `spin`, `bob`, `bob_speed`, `pulse`, `pulse_speed`, `on_click`, `off` |
| `light` | **Light and atmosphere of the 3D scene** (Unity, Canvas path). `kind=sun` is the key light with shadows, `fill` the shadow-side readability light, `point` a lamp/fire, `fog` the depth that turns the far plane into a backdrop a figure reads against, `sky` the ambient gradient. Without a scene the command is ignored. | `kind`, `id`, `angle`, `pos`, `color`, `power`, `range`, `near`, `far`, `top`, `bottom`, `off` |
...
| ✅ **Пути к картинкам принимают подстановку.** | `sprite_url="…/skeleton_{поза}.png"`, `bg sprite_url="/bg/{погода}_двор.jpg"` — раньше каждая поза/дистанция/время суток требовали отдельной ветки с почти одинаковой строкой (в зонной боёвке — 27 веток на врага). Для персонажей ту же задачу решает каталог `cast`; подстановка — для всего остального. |
| ✅ **Глубина сцены: `depth`.** | Одно число вместо ручной пары «размер + порядок» на каждую дистанцию: `actor skeleton depth=6` — размер считается перспективой, дальний сам уходит за ближнего, а при движении камеры получается параллакс (дальние отстают). Работает и на плоском фоне. |
| ✅ **Привязка к 3D-набору: `world`.** | Спрайт стоит в ТОЧКЕ набора: камера `bg3d` едет — фигура остаётся у своей колонны, а не плывёт по экрану. Экранное место и глубина считаются проекцией камеры набора. Без активного `bg3d` поле игнорируется. |
| ✅ **Процент экрана принимается везде.** | `x=26%` работает и в `obj` (родные единицы — доли), и в лейбле (родные — проценты). Голое число сохраняет прежний смысл поля. |
| ✅ **Полоса прогресса: `fill` (0..1) и `fill_from`.** | Спрайт ОБРЕЗАЕТСЯ, а не сжимается — скругления и рамка остаются в масштабе. `fill` анимируется как любое свойство (`anim id=bar prop=fill to="{hp/hp_max}" dur=0.25`), а второй спрайт с бо́льшим `dur` даёт «полосу урона» из Dota. |
| ✅ **Числовые поля `obj`/`actor` принимают ВЫРАЖЕНИЯ.** | `fill="{hp / hp_max}"`, `width="{0.2 + k}"` — шаблон в кавычках вычисляется по живым переменным при каждом применении команды, как текст лейбла. Раньше принимался только литерал, и любая величина от состояния писалась лестницей веток. Пересчёт происходит на команде: чтобы полоса обновилась, перевызовите ту же `obj`-команду (обычно это одна процедура вроде `hp_bar()`). |
