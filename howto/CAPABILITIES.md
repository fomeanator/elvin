...
| `bg3d` | **A built 3D set instead of painted art** (Unity, Canvas path). `bg3d id=…` stands the set; later `bg3d` calls without an id just move its camera — one room yields as many angles as the script asks for. `off` returns to flat backgrounds. A still set is filmed once and then costs nothing — the engine only films every frame when the set actually moves (animation, particles, scripts), which it detects itself; `live=1`/`live=0` forces that choice when motion hides in a shader. Needs the game to implement prefab loading (`ILvnAssets.LoadPrefabAsync`); without it the scene keeps the background it had, so the script still plays. | `id`/`prefab`/`scene`, `x y z`, `pitch yaw`, `fov`, `dur`, `off`, `live` |
